...
final class DefaultPagerNestedScrollConnection implements NestedScrollConnection {
    ...
    public final long m1217consumeOnOrientationQWom1Mo(long j, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return Velocity.m7230copyOhffZ5M$default(j, 0.0f, 0.0f, 2, null);
        }
        return Velocity.m7230copyOhffZ5M$default(j, 0.0f, 0.0f, 1, null);
    }

    @Override // androidx.compose.ui.input.nestedscroll.NestedScrollConnection
    /* renamed from: onPreScroll-OzD1aCk */
    public long mo1061onPreScrollOzD1aCk(long available, int source) {
        if (NestedScrollSource.m5605equalsimpl0(source, NestedScrollSource.INSTANCE.m5617getUserInputWNlRxjI()) && Math.abs(this.state.getCurrentPageOffsetFraction()) > 1.0E-6d) {
            ...
            float f = -this.state.dispatchRawDelta(-RangesKt.coerceIn(this.orientation == Orientation.Horizontal ? Offset.m4274getXimpl(available) : Offset.m4275getYimpl(available), currentPageOffsetFraction, pageSize));
            float m4274getXimpl = this.orientation == Orientation.Horizontal ? f : Offset.m4274getXimpl(available);
            if (this.orientation != Orientation.Vertical) {
                f = Offset.m4275getYimpl(available);
            }
            return Offset.m4267copydBAh8RU(available, m4274getXimpl, f);
        }
    ...
    public long mo787onPostScrollDzOQY0M(long consumed, long available, int source) {
        if (NestedScrollSource.m5605equalsimpl0(source, NestedScrollSource.INSTANCE.m5616getSideEffectWNlRxjI()) && m1216mainAxisk4lQ0M(available) != 0.0f) {
    ...
    public Object mo786onPostFlingRZ2iAVY(long j, long j2, Continuation<? super Velocity> continuation) {
        return Velocity.m7225boximpl(m1217consumeOnOrientationQWom1Mo(j2, this.orientation));
    }
