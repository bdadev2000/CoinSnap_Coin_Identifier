...
public final class TextFieldMagnifierKt {
    ...
    public static final long m1549calculateSelectionMagnifierCenterAndroidhUlJWOE(TransformedTextFieldState transformedTextFieldState, TextFieldSelectionState textFieldSelectionState, TextLayoutState textLayoutState, long j) {
        ...
        long m1573getHandleDragPositionF1C5BW0 = textFieldSelectionState.m1573getHandleDragPositionF1C5BW0();
        if (OffsetKt.m4295isUnspecifiedk4lQ0M(m1573getHandleDragPositionF1C5BW0) || transformedTextFieldState.getVisualText().length() == 0) {
        ...
        if (i == 1 || i == 2) {
            m6460getStartimpl = TextRange.m6460getStartimpl(selection);
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            m6460getStartimpl = TextRange.m6455getEndimpl(selection);
        }
        TextLayoutResult layoutResult = textLayoutState.getLayoutResult();
        if (layoutResult == null) {
            return Offset.INSTANCE.m4289getUnspecifiedF1C5BW0();
        }
        float m4274getXimpl = Offset.m4274getXimpl(m1573getHandleDragPositionF1C5BW0);
        int lineForOffset = layoutResult.getLineForOffset(m6460getStartimpl);
        float lineLeft = layoutResult.getLineLeft(lineForOffset);
        float lineRight = layoutResult.getLineRight(lineForOffset);
        float coerceIn = RangesKt.coerceIn(m4274getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
        ...
        if (textLayoutNodeCoordinates != null) {
            ...
            if (textLayoutNodeCoordinates != null) {
                Offset = TextLayoutStateKt.m1524coerceIn3MmeM6k(Offset, SelectionManagerKt.visibleBounds(textLayoutNodeCoordinates));
            }
        }
        return TextLayoutStateKt.m1526fromTextLayoutToCoreUv8p0NA(textLayoutState, Offset);
    }
}
