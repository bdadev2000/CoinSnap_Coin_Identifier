...
public final class TextFieldTextLayoutModifierNode extends Modifier.Node implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo395measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult m1522layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1522layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable mo5851measureBRTryo0 = measurable.mo5851measureBRTryo0(Constraints.INSTANCE.m6959fitPrioritizingWidthZbe2FdA(IntSize.m7167getWidthimpl(m1522layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m7167getWidthimpl(m1522layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m7166getHeightimpl(m1522layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m7166getHeightimpl(m1522layoutWithNewMeasureInputshBUhpc.getSize())));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            m6997constructorimpl = measureScope.mo674toDpu2uoSUM(TextDelegateKt.ceilToIntPx(m1522layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            m6997constructorimpl = Dp.m6997constructorimpl(0);
        }
        textLayoutState.m1523setMinHeightForSingleLineField0680j_4(m6997constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(m1522layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(m1522layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        int m7167getWidthimpl = IntSize.m7167getWidthimpl(m1522layoutWithNewMeasureInputshBUhpc.getSize());
        int m7166getHeightimpl = IntSize.m7166getHeightimpl(m1522layoutWithNewMeasureInputshBUhpc.getSize());
        Map<AlignmentLine, Integer> map = this.baselineCache;
        Intrinsics.checkNotNull(map);
        return measureScope.layout(m7167getWidthimpl, m7166getHeightimpl, map, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldTextLayoutModifierNode$measure$1
