...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m6456getLengthimpl(textFieldBuffer.getChanges().mo1410getRangejx7JFs(0)) != 1 || TextRange.m6456getLengthimpl(textFieldBuffer.getChanges().mo1409getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int m6458getMinimpl = TextRange.m6458getMinimpl(textFieldBuffer.getChanges().mo1410getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != m6458getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(m6458getMinimpl);
        }
    }
