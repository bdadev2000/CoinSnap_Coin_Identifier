...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            m7125getXimpl = IntOffset.m7126getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            m7125getXimpl = IntOffset.m7125getXimpl(lazyGridItemInfo2.getOffset());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (intValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == intValue) {
                    if (z) {
                        m7167getWidthimpl = IntSize.m7166getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        m7167getWidthimpl = IntSize.m7167getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    i4 = Math.max(i4, m7167getWidthimpl);
                    i++;
                }
                i2 += i4;
                i3++;
