...
public abstract class RippleIndicationInstance implements IndicationInstance {
    ...
    public final void m2065drawStateLayerH2RKhps(DrawScope drawScope, float f, long j) {
        float f2;
        StateLayer stateLayer = this.stateLayer;
        if (Float.isNaN(f)) {
            f2 = RippleAnimationKt.m2060getRippleEndRadiuscSwnlzA(drawScope, this.bounded, drawScope.mo5071getSizeNHjbRc());
        } else {
            f2 = drawScope.mo677toPx0680j_4(f);
        }
        stateLayer.m2073drawStateLayermxwnekA(drawScope, f2, j);
    }
}
