...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo2057rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer composer, int i) {
        long mo1893defaultColorWaAFU9c;
        ...
        if (this.color.getValue().m4525unboximpl() != Color.INSTANCE.m4551getUnspecified0d7_KjU()) {
            composer.startReplaceGroup(-303571590);
            composer.endReplaceGroup();
            mo1893defaultColorWaAFU9c = this.color.getValue().m4525unboximpl();
        } else {
            composer.startReplaceGroup(-303521246);
            ComposerKt.sourceInformation(composer, "203@9725L14");
            mo1893defaultColorWaAFU9c = rippleTheme.mo1893defaultColorWaAFU9c(composer, 0);
            composer.endReplaceGroup();
        }
        State<Color> rememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Color.m4505boximpl(mo1893defaultColorWaAFU9c), composer, 0);
        ...
        int i2 = i & 14;
        RippleIndicationInstance mo2057rememberUpdatedRippleInstance942rkJo = mo2057rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, rememberUpdatedState, rememberUpdatedState2, composer, i2 | ((i << 12) & 458752));
        ComposerKt.sourceInformationMarkerStart(composer, -9776216, "CC(remember):Ripple.kt#9igjgp");
        boolean changedInstance = composer.changedInstance(mo2057rememberUpdatedRippleInstance942rkJo) | (((i2 ^ 6) > 4 && composer.changed(interactionSource)) || (i & 6) == 4);
        Ripple$rememberUpdatedInstance$1$1 rememberedValue = composer.rememberedValue();
        if (changedInstance || rememberedValue == Composer.INSTANCE.getEmpty()) {
            rememberedValue = new Ripple$rememberUpdatedInstance$1$1(interactionSource, mo2057rememberUpdatedRippleInstance942rkJo, null);
            composer.updateRememberedValue(rememberedValue);
        }
        ComposerKt.sourceInformationMarkerEnd(composer);
        EffectsKt.LaunchedEffect(mo2057rememberUpdatedRippleInstance942rkJo, interactionSource, (Function2) rememberedValue, composer, (i << 3) & StatusBarUtils.DEFAULT_STATUS_BAR_ALPHA);
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        composer.endReplaceGroup();
        return mo2057rememberUpdatedRippleInstance942rkJo;
    }
    ...
    public boolean equals(Object other) {
        ...
        return this.bounded == ripple.bounded && Dp.m7002equalsimpl0(this.radius, ripple.radius) && Intrinsics.areEqual(this.color, ripple.color);
    }

    public int hashCode() {
        return (((Boolean.hashCode(this.bounded) * 31) + Dp.m7003hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
