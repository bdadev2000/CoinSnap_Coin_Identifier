...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int i;
        this.rippleSize = contentDrawScope.mo5071getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m2060getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo5071getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo671roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m4525unboximpl = this.color.getValue().m4525unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m2065drawStateLayerH2RKhps(contentDrawScope2, this.radius, m4525unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m2064setRippleProperties07v42R4(contentDrawScope.mo5071getSizeNHjbRc(), m4525unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m2063addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m4525unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
