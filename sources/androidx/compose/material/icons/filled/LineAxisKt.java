package androidx.compose.material.icons.filled;

import androidx.compose.ui.graphics.vector.ImageVector;
import androidx.constraintlayout.widget.ConstraintLayout;
import kotlin.Metadata;

/* compiled from: LineAxis.kt */
@Metadata(d1 = {"\u0000\u0010\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\"\u0010\u0010\u0000\u001a\u0004\u0018\u00010\u0001X\u0082\u000e¢\u0006\u0002\n\u0000\"\u0015\u0010\u0002\u001a\u00020\u0001*\u00020\u00038F¢\u0006\u0006\u001a\u0004\b\u0004\u0010\u0005¨\u0006\u0006"}, d2 = {"_lineAxis", "Landroidx/compose/ui/graphics/vector/ImageVector;", "LineAxis", "Landroidx/compose/material/icons/Icons$Filled;", "getLineAxis", "(Landroidx/compose/material/icons/Icons$Filled;)Landroidx/compose/ui/graphics/vector/ImageVector;", "material-icons-extended_release"}, k = 2, mv = {1, 8, 0}, xi = ConstraintLayout.LayoutParams.Table.LAYOUT_CONSTRAINT_VERTICAL_CHAINSTYLE)
/* loaded from: classes.dex */
public final class LineAxisKt {
    private static ImageVector _lineAxis;

    /*  JADX ERROR: JadxRuntimeException in pass: InlineMethods
        jadx.core.utils.exceptions.JadxRuntimeException: Failed to process method for inline: androidx.compose.ui.graphics.vector.ImageVector.Builder.addPath-oIyEayM$default(androidx.compose.ui.graphics.vector.ImageVector$Builder, java.util.List, int, java.lang.String, androidx.compose.ui.graphics.Brush, float, androidx.compose.ui.graphics.Brush, float, float, int, int, float, float, float, float, int, java.lang.Object):androidx.compose.ui.graphics.vector.ImageVector$Builder
        	at jadx.core.dex.visitors.InlineMethods.processInvokeInsn(InlineMethods.java:74)
        	at jadx.core.dex.visitors.InlineMethods.visit(InlineMethods.java:49)
        Caused by: java.lang.NullPointerException
        */
    public static final androidx.compose.ui.graphics.vector.ImageVector getLineAxis(androidx.compose.material.icons.Icons.Filled r30) {
        /*
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.filled.LineAxisKt._lineAxis
            if (r0 == 0) goto L8
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        L8:
            androidx.compose.ui.graphics.vector.ImageVector$Builder r1 = new androidx.compose.ui.graphics.vector.ImageVector$Builder
            r13 = r1
            r0 = 1103101952(0x41c00000, float:24.0)
            float r3 = androidx.compose.ui.unit.Dp.m6997constructorimpl(r0)
            float r4 = androidx.compose.ui.unit.Dp.m6997constructorimpl(r0)
            r11 = 96
            r12 = 0
            java.lang.String r2 = "Filled.LineAxis"
            r5 = 1103101952(0x41c00000, float:24.0)
            r6 = 1103101952(0x41c00000, float:24.0)
            r7 = 0
            r9 = 0
            r10 = 0
            r1.<init>(r2, r3, r4, r5, r6, r7, r9, r10, r11, r12)
            int r15 = androidx.compose.ui.graphics.vector.VectorKt.getDefaultFillType()
            androidx.compose.ui.graphics.SolidColor r0 = new androidx.compose.ui.graphics.SolidColor
            androidx.compose.ui.graphics.Color$Companion r1 = androidx.compose.ui.graphics.Color.INSTANCE
            long r1 = r1.m4541getBlack0d7_KjU()
            r3 = 0
            r0.<init>(r1, r3)
            r17 = r0
            androidx.compose.ui.graphics.Brush r17 = (androidx.compose.ui.graphics.Brush) r17
            androidx.compose.ui.graphics.StrokeCap$Companion r0 = androidx.compose.ui.graphics.StrokeCap.INSTANCE
            int r22 = r0.m4868getButtKaPHkGw()
            androidx.compose.ui.graphics.StrokeJoin$Companion r0 = androidx.compose.ui.graphics.StrokeJoin.INSTANCE
            int r23 = r0.m4878getBevelLxFBmk8()
            androidx.compose.ui.graphics.vector.PathBuilder r0 = new androidx.compose.ui.graphics.vector.PathBuilder
            r0.<init>()
            r1 = 1102053376(0x41b00000, float:22.0)
            r2 = 1089323663(0x40edc28f, float:7.43)
            r0.moveTo(r1, r2)
            r1 = -1078691103(0xffffffffbfb47ae1, float:-1.41)
            r0.lineToRelative(r1, r1)
            r2 = -1065290301(0xffffffffc080f5c3, float:-4.03)
            r3 = 1083241923(0x4090f5c3, float:4.53)
            r0.lineToRelative(r2, r3)
            r2 = -1058935931(0xffffffffc0e1eb85, float:-7.06)
            r3 = -1060005478(0xffffffffc0d1999a, float:-6.55)
            r0.lineToRelative(r2, r3)
            r2 = -1058013184(0xffffffffc0f00000, float:-7.5)
            r3 = 1089491436(0x40f051ec, float:7.51)
            r0.lineToRelative(r2, r3)
            r4 = 1069547520(0x3fc00000, float:1.5)
            r0.lineToRelative(r4, r4)
            r5 = 1086618337(0x40c47ae1, float:6.14)
            r6 = -1060844339(0xffffffffc0c4cccd, float:-6.15)
            r0.lineToRelative(r5, r6)
            r5 = 1085464904(0x40b2e148, float:5.59)
            r6 = 1084605071(0x40a5c28f, float:5.18)
            r0.lineToRelative(r5, r6)
            r5 = -1076006748(0xffffffffbfdd70a4, float:-1.73)
            r6 = 1073322394(0x3ff9999a, float:1.95)
            r0.lineToRelative(r5, r6)
            r5 = -1065353216(0xffffffffc0800000, float:-4.0)
            r0.lineToRelative(r5, r5)
            r0.lineToRelative(r2, r3)
            r0.lineToRelative(r4, r4)
            r2 = 1086324736(0x40c00000, float:6.0)
            r3 = -1061137940(0xffffffffc0c051ec, float:-6.01)
            r0.lineToRelative(r2, r3)
            r2 = 1082130432(0x40800000, float:4.0)
            r0.lineToRelative(r2, r2)
            r2 = 1078733046(0x404c28f6, float:3.19)
            r3 = -1067072881(0xffffffffc065c28f, float:-3.59)
            r0.lineToRelative(r2, r3)
            r2 = 1081711002(0x4079999a, float:3.9)
            r3 = 1080494653(0x40670a3d, float:3.61)
            r0.lineToRelative(r2, r3)
            r2 = 1068792545(0x3fb47ae1, float:1.41)
            r0.lineToRelative(r2, r1)
            r1 = -1065437102(0xffffffffc07eb852, float:-3.98)
            r2 = -1066611507(0xffffffffc06ccccd, float:-3.7)
            r0.lineToRelative(r1, r2)
            r0.close()
            java.util.List r14 = r0.getNodes()
            r28 = 14336(0x3800, float:2.0089E-41)
            r29 = 0
            java.lang.String r16 = ""
            r18 = 1065353216(0x3f800000, float:1.0)
            r19 = 0
            r20 = 1065353216(0x3f800000, float:1.0)
            r21 = 1065353216(0x3f800000, float:1.0)
            r24 = 1065353216(0x3f800000, float:1.0)
            r25 = 0
            r26 = 0
            r27 = 0
            androidx.compose.ui.graphics.vector.ImageVector$Builder r0 = androidx.compose.ui.graphics.vector.ImageVector.Builder.m5206addPathoIyEayM$default(r13, r14, r15, r16, r17, r18, r19, r20, r21, r22, r23, r24, r25, r26, r27, r28, r29)
            androidx.compose.ui.graphics.vector.ImageVector r0 = r0.build()
            androidx.compose.material.icons.filled.LineAxisKt._lineAxis = r0
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: androidx.compose.material.icons.filled.LineAxisKt.getLineAxis(androidx.compose.material.icons.Icons$Filled):androidx.compose.ui.graphics.vector.ImageVector");
    }
}
