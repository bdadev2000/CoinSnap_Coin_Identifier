package androidx.compose.material.icons.filled;

import androidx.compose.ui.graphics.vector.ImageVector;
import androidx.constraintlayout.widget.ConstraintLayout;
import kotlin.Metadata;

/* compiled from: BorderColor.kt */
@Metadata(d1 = {"\u0000\u0010\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\"\u0010\u0010\u0000\u001a\u0004\u0018\u00010\u0001X\u0082\u000e¢\u0006\u0002\n\u0000\"\u0015\u0010\u0002\u001a\u00020\u0001*\u00020\u00038F¢\u0006\u0006\u001a\u0004\b\u0004\u0010\u0005¨\u0006\u0006"}, d2 = {"_borderColor", "Landroidx/compose/ui/graphics/vector/ImageVector;", "BorderColor", "Landroidx/compose/material/icons/Icons$Filled;", "getBorderColor", "(Landroidx/compose/material/icons/Icons$Filled;)Landroidx/compose/ui/graphics/vector/ImageVector;", "material-icons-extended_release"}, k = 2, mv = {1, 8, 0}, xi = ConstraintLayout.LayoutParams.Table.LAYOUT_CONSTRAINT_VERTICAL_CHAINSTYLE)
/* loaded from: classes.dex */
public final class BorderColorKt {
    private static ImageVector _borderColor;

    /*  JADX ERROR: JadxRuntimeException in pass: InlineMethods
        jadx.core.utils.exceptions.JadxRuntimeException: Failed to process method for inline: androidx.compose.ui.graphics.vector.ImageVector.Builder.addPath-oIyEayM$default(androidx.compose.ui.graphics.vector.ImageVector$Builder, java.util.List, int, java.lang.String, androidx.compose.ui.graphics.Brush, float, androidx.compose.ui.graphics.Brush, float, float, int, int, float, float, float, float, int, java.lang.Object):androidx.compose.ui.graphics.vector.ImageVector$Builder
        	at jadx.core.dex.visitors.InlineMethods.processInvokeInsn(InlineMethods.java:74)
        	at jadx.core.dex.visitors.InlineMethods.visit(InlineMethods.java:49)
        Caused by: java.lang.NullPointerException: Cannot invoke "jadx.core.dex.instructions.args.RegisterArg.getParentInsn()" because "reg" is null
        	at jadx.core.dex.instructions.args.SSAVar.updateUsedInPhiList(SSAVar.java:162)
        	at jadx.core.dex.nodes.InsnNode.rebindArgs(InsnNode.java:490)
        	at jadx.core.dex.instructions.mods.TernaryInsn.rebindArgs(TernaryInsn.java:92)
        	at jadx.core.dex.nodes.InsnNode.rebindArgs(InsnNode.java:492)
        	at jadx.core.utils.BlockUtils.replaceInsn(BlockUtils.java:1109)
        	at jadx.core.utils.BlockUtils.replaceInsn(BlockUtils.java:1118)
        	at jadx.core.dex.visitors.InlineMethods.inlineMethod(InlineMethods.java:113)
        	at jadx.core.dex.visitors.InlineMethods.processInvokeInsn(InlineMethods.java:72)
        	... 1 more
        */
    public static final androidx.compose.ui.graphics.vector.ImageVector getBorderColor(androidx.compose.material.icons.Icons.Filled r30) {
        /*
            androidx.compose.ui.graphics.vector.ImageVector r0 = androidx.compose.material.icons.filled.BorderColorKt._borderColor
            if (r0 == 0) goto L8
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        L8:
            androidx.compose.ui.graphics.vector.ImageVector$Builder r1 = new androidx.compose.ui.graphics.vector.ImageVector$Builder
            r13 = r1
            r0 = 1103101952(0x41c00000, float:24.0)
            float r3 = androidx.compose.ui.unit.Dp.m6997constructorimpl(r0)
            float r4 = androidx.compose.ui.unit.Dp.m6997constructorimpl(r0)
            r11 = 96
            r12 = 0
            java.lang.String r2 = "Filled.BorderColor"
            r5 = 1103101952(0x41c00000, float:24.0)
            r6 = 1103101952(0x41c00000, float:24.0)
            r7 = 0
            r9 = 0
            r10 = 0
            r1.<init>(r2, r3, r4, r5, r6, r7, r9, r10, r11, r12)
            int r15 = androidx.compose.ui.graphics.vector.VectorKt.getDefaultFillType()
            androidx.compose.ui.graphics.SolidColor r1 = new androidx.compose.ui.graphics.SolidColor
            androidx.compose.ui.graphics.Color$Companion r2 = androidx.compose.ui.graphics.Color.INSTANCE
            long r2 = r2.m4541getBlack0d7_KjU()
            r4 = 0
            r1.<init>(r2, r4)
            r17 = r1
            androidx.compose.ui.graphics.Brush r17 = (androidx.compose.ui.graphics.Brush) r17
            androidx.compose.ui.graphics.StrokeCap$Companion r1 = androidx.compose.ui.graphics.StrokeCap.INSTANCE
            int r22 = r1.m4868getButtKaPHkGw()
            androidx.compose.ui.graphics.StrokeJoin$Companion r1 = androidx.compose.ui.graphics.StrokeJoin.INSTANCE
            int r23 = r1.m4878getBevelLxFBmk8()
            androidx.compose.ui.graphics.vector.PathBuilder r8 = new androidx.compose.ui.graphics.vector.PathBuilder
            r8.<init>()
            r1 = 1102053376(0x41b00000, float:22.0)
            r8.moveTo(r1, r0)
            r1 = 1073741824(0x40000000, float:2.0)
            r8.horizontalLineTo(r1)
            r1 = -1065353216(0xffffffffc0800000, float:-4.0)
            r8.verticalLineToRelative(r1)
            r1 = 1101004800(0x41a00000, float:20.0)
            r8.horizontalLineToRelative(r1)
            r8.verticalLineTo(r0)
            r8.close()
            r0 = 1095824835(0x4150f5c3, float:13.06)
            r1 = 1084626043(0x40a6147b, float:5.19)
            r8.moveTo(r0, r1)
            r2 = 1081081856(0x40700000, float:3.75)
            r8.lineToRelative(r2, r2)
            r2 = 1089994752(0x40f80000, float:7.75)
            r3 = 1099956224(0x41900000, float:18.0)
            r8.lineTo(r2, r3)
            r2 = 1082130432(0x40800000, float:4.0)
            r8.horizontalLineTo(r2)
            r2 = -1066401792(0xffffffffc0700000, float:-3.75)
            r8.verticalLineToRelative(r2)
            r8.lineTo(r0, r1)
            r8.close()
            r0 = 1099893309(0x418f0a3d, float:17.88)
            r9 = 1090246410(0x40fbd70a, float:7.87)
            r8.moveTo(r0, r9)
            r8.lineToRelative(r2, r2)
            r1 = 1072315761(0x3fea3d71, float:1.83)
            r2 = -1075167887(0xffffffffbfea3d71, float:-1.83)
            r8.lineToRelative(r1, r2)
            r6 = 1068792545(0x3fb47ae1, float:1.41)
            r7 = 0
            r2 = 1053273620(0x3ec7ae14, float:0.39)
            r3 = -1094210028(0xffffffffbec7ae14, float:-0.39)
            r4 = 1065520988(0x3f828f5c, float:1.02)
            r5 = -1094210028(0xffffffffbec7ae14, float:-0.39)
            r1 = r8
            r1.curveToRelative(r2, r3, r4, r5, r6, r7)
            r1 = 1075167887(0x4015c28f, float:2.34)
            r8.lineToRelative(r1, r1)
            r6 = 0
            r7 = 1068792545(0x3fb47ae1, float:1.41)
            r3 = 1053273620(0x3ec7ae14, float:0.39)
            r4 = 1053273620(0x3ec7ae14, float:0.39)
            r5 = 1065520988(0x3f828f5c, float:1.02)
            r1 = r8
            r1.curveToRelative(r2, r3, r4, r5, r6, r7)
            r8.lineTo(r0, r9)
            r8.close()
            java.util.List r14 = r8.getNodes()
            r28 = 14336(0x3800, float:2.0089E-41)
            r29 = 0
            java.lang.String r16 = ""
            r18 = 1065353216(0x3f800000, float:1.0)
            r19 = 0
            r20 = 1065353216(0x3f800000, float:1.0)
            r21 = 1065353216(0x3f800000, float:1.0)
            r24 = 1065353216(0x3f800000, float:1.0)
            r25 = 0
            r26 = 0
            r27 = 0
            androidx.compose.ui.graphics.vector.ImageVector$Builder r0 = androidx.compose.ui.graphics.vector.ImageVector.Builder.m5206addPathoIyEayM$default(r13, r14, r15, r16, r17, r18, r19, r20, r21, r22, r23, r24, r25, r26, r27, r28, r29)
            androidx.compose.ui.graphics.vector.ImageVector r0 = r0.build()
            androidx.compose.material.icons.filled.BorderColorKt._borderColor = r0
            kotlin.jvm.internal.Intrinsics.checkNotNull(r0)
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: androidx.compose.material.icons.filled.BorderColorKt.getBorderColor(androidx.compose.material.icons.Icons$Filled):androidx.compose.ui.graphics.vector.ImageVector");
    }
}
