...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m6238toOffsetUv8p0NA(int[] iArr, long j) {
        float coerceAtLeast;
        float coerceAtLeast2;
        if (Offset.m4274getXimpl(j) >= 0.0f) {
            coerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m4274getXimpl(j));
        } else {
            coerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m4274getXimpl(j));
        }
        if (Offset.m4275getYimpl(j) >= 0.0f) {
            coerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m4275getYimpl(j));
        } else {
            coerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m4275getYimpl(j));
        }
    ...
    public static final int m6239toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m5605equalsimpl0(i, NestedScrollSource.INSTANCE.m5617getUserInputWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m6237getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m4274getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m4275getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
