...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    private TargetState<T> state = new TargetState<>(getAnimation().m6920getAnimationObject().getValue(), getAnimation().m6920getAnimationObject().getValue());
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m6920getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m6920getAnimationObject().getVelocity());
    }
}
