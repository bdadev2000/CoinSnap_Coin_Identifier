...
public abstract class NodeCoordinator extends LookaheadCapablePlaceable implements Measurable, LayoutCoordinates, OwnerScope {
    ...
    private static final float[] tmpMatrix = Matrix.m4746constructorimpl$default(null, 1, null);
    private static final HitTestSource PointerInputSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$PointerInputSource$1
        ...
        public void mo6081childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            layoutNode.m6003hitTestM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw, reason: not valid java name */
        public int mo6082entityTypeOLwlOKw() {
            return NodeKind.m6086constructorimpl(16);
        }
    ...
    private static final HitTestSource SemanticsSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$SemanticsSource$1
        ...
        public void mo6081childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            layoutNode.m6004hitTestSemanticsM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw */
        public int mo6082entityTypeOLwlOKw() {
            return NodeKind.m6086constructorimpl(8);
        }
    };
    private Density layerDensity = getLayoutNode().getDensity();
    private LayoutDirection layerLayoutDirection = getLayoutNode().getLayoutDirection();
    private float lastLayerAlpha = 0.8f;
    private long position = IntOffset.INSTANCE.m7135getZeronOccac();
    ...
    public interface HitTestSource {
        /* renamed from: childHitTest-YqVAtuI */
        void mo6081childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer);

        /* renamed from: entityType-OLwlOKw */
        int mo6082entityTypeOLwlOKw();
    ...
    public final /* synthetic */ <T> void m6079visitNodesaLcG6gQ(int type, Function1<? super T, Unit> block) {
        boolean m6095getIncludeSelfInTraversalH91voCI = NodeKindKt.m6095getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m6095getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return;
        }
        for (Modifier.Node headNode = headNode(m6095getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & type) != 0; headNode = headNode.getChild()) {
    ...
    private final boolean m6058hasNodeH91voCI(int type) {
        Modifier.Node headNode = headNode(NodeKindKt.m6095getIncludeSelfInTraversalH91voCI(type));
        return headNode != null && DelegatableNodeKt.m5971has64DMado(headNode, type);
    }

    /* renamed from: head-H91voCI */
    public final Modifier.Node m6072headH91voCI(int type) {
        boolean m6095getIncludeSelfInTraversalH91voCI = NodeKindKt.m6095getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m6095getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return null;
    ...
    public final long mo5859getSizeYbymL2g() {
    ...
    public void replace$ui_release() {
        ...
        if (graphicsLayer != null) {
            mo5912placeAtf8xVGno(getPosition(), this.zIndex, graphicsLayer);
        } else {
            mo5852placeAtf8xVGno(getPosition(), this.zIndex, this.layerBlock);
        }
    }
    ...
    protected void onMeasureResultChanged(int width, int height) {
        ...
        if (ownedLayer != null) {
            ownedLayer.mo6137resizeozmzZPI(IntSizeKt.IntSize(width, height));
        } else if (getLayoutNode().isPlaced() && (nodeCoordinator = this.wrappedBy) != null) {
            nodeCoordinator.invalidateLayer();
        }
        m5913setMeasuredSizeozmzZPI(IntSizeKt.IntSize(width, height));
        if (this.layerBlock != null) {
            updateLayerParameters(false);
        }
        int m6086constructorimpl = NodeKind.m6086constructorimpl(4);
        boolean m6095getIncludeSelfInTraversalH91voCI = NodeKindKt.m6095getIncludeSelfInTraversalH91voCI(m6086constructorimpl);
        Modifier.Node tail = getTail();
        if (m6095getIncludeSelfInTraversalH91voCI || (tail = tail.getParent()) != null) {
            for (Modifier.Node headNode = headNode(m6095getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & m6086constructorimpl) != 0; headNode = headNode.getChild()) {
                if ((headNode.getKindSet() & m6086constructorimpl) != 0) {
    ...
    protected void m6077setPositiongyyYBs(long j) {
        this.position = j;
    }
    ...
    public Object getParentData() {
        if (!getLayoutNode().getNodes().m6047hasH91voCI$ui_release(NodeKind.m6086constructorimpl(64))) {
            return null;
        }
        getTail();
        Ref.ObjectRef objectRef = new Ref.ObjectRef();
        for (Modifier.Node tail = getLayoutNode().getNodes().getTail(); tail != null; tail = tail.getParent()) {
            if ((NodeKind.m6086constructorimpl(64) & tail.getKindSet()) != 0) {
                int m6086constructorimpl = NodeKind.m6086constructorimpl(64);
                MutableVector mutableVector = null;
    ...
    public final long m6070getLastMeasurementConstraintsmsEJaDk$ui_release() {
        return getMeasurementConstraints();
    }

    /* renamed from: performingMeasure-K40F9xA */
    protected final Placeable m6075performingMeasureK40F9xA(long constraints, Function0<? extends Placeable> block) {
        m5914setMeasurementConstraintsBRTryo0(constraints);
        return block.invoke();
    }

    @Override // androidx.compose.ui.layout.Placeable
    /* renamed from: placeAt-f8xVGno */
    public void mo5852placeAtf8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        if (this.forcePlaceWithLookaheadOffset) {
            LookaheadDelegate lookaheadDelegate = getLookaheadDelegate();
            Intrinsics.checkNotNull(lookaheadDelegate);
            m6062placeSelfMLgxB_4(lookaheadDelegate.getPosition(), zIndex, layerBlock, null);
            return;
        }
        m6062placeSelfMLgxB_4(position, zIndex, layerBlock, null);
    }

    @Override // androidx.compose.ui.layout.Placeable
    /* renamed from: placeAt-f8xVGno */
    public void mo5912placeAtf8xVGno(long position, float zIndex, GraphicsLayer layer) {
        if (this.forcePlaceWithLookaheadOffset) {
            LookaheadDelegate lookaheadDelegate = getLookaheadDelegate();
            Intrinsics.checkNotNull(lookaheadDelegate);
            m6062placeSelfMLgxB_4(lookaheadDelegate.getPosition(), zIndex, null, layer);
            return;
        }
        m6062placeSelfMLgxB_4(position, zIndex, null, layer);
    }

    /* renamed from: placeSelf-MLgxB_4 */
    private final void m6062placeSelfMLgxB_4(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock, GraphicsLayer explicitLayer) {
        if (explicitLayer != null) {
            ...
            if (this.layer == null) {
                ...
                createLayer.mo6137resizeozmzZPI(getMeasuredSize());
                createLayer.mo6136movegyyYBs(position);
        ...
        if (!IntOffset.m7124equalsimpl0(getPosition(), position)) {
            m6077setPositiongyyYBs(position);
            getLayoutNode().getLayoutDelegate().getMeasurePassDelegate().notifyChildrenUsingCoordinatesWhilePlacing();
            OwnedLayer ownedLayer = this.layer;
            if (ownedLayer != null) {
                ownedLayer.mo6136movegyyYBs(position);
            } else {
                NodeCoordinator nodeCoordinator = this.wrappedBy;
                if (nodeCoordinator != null) {
                    nodeCoordinator.invalidateLayer();
    ...
    public final void m6076placeSelfApparentToRealOffsetMLgxB_4(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock, GraphicsLayer layer) {
        m6062placeSelfMLgxB_4(IntOffset.m7129plusqkQi6aY(position, getApparentToRealOffset()), zIndex, layerBlock, layer);
    }
    ...
    public final void draw(Canvas canvas, GraphicsLayer graphicsLayer) {
        ...
        float m7125getXimpl = IntOffset.m7125getXimpl(getPosition());
        float m7126getYimpl = IntOffset.m7126getYimpl(getPosition());
    ...
    public final void updateLayerBlock(Function1<? super GraphicsLayerScope, Unit> layerBlock, boolean forceUpdateLayerParameters) {
        ...
        if (layoutNode.isAttached() && layerBlock != null) {
            ...
            createLayer$default.mo6137resizeozmzZPI(getMeasuredSize());
            createLayer$default.mo6136movegyyYBs(getPosition());
    ...
    private final void updateLayerParameters(boolean invokeOnLayoutChange) {
        ...
        if (ownedLayer != null) {
            ...
            if (function1 != null) {
                ...
                reusableGraphicsLayerScope.m4827setSizeuvyYCjk(IntSizeKt.m7179toSizeozmzZPI(mo5859getSizeYbymL2g()));
    ...
    public final long m6071getMinimumTouchTargetSizeNHjbRc() {
        return this.layerDensity.mo678toSizeXkaWNTQ(getLayoutNode().getViewConfiguration().mo6007getMinimumTouchTargetSizeMYxV2XQ());
    }

    /* renamed from: hitTest-YqVAtuI */
    public final void m6073hitTestYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        Modifier.Node m6072headH91voCI = m6072headH91voCI(hitTestSource.mo6082entityTypeOLwlOKw());
        if (!m6080withinLayerBoundsk4lQ0M(pointerPosition)) {
            if (isTouchEvent) {
                float m6068distanceInMinimumTouchTargettz77jQw = m6068distanceInMinimumTouchTargettz77jQw(pointerPosition, m6071getMinimumTouchTargetSizeNHjbRc());
                if (Float.isInfinite(m6068distanceInMinimumTouchTargettz77jQw) || Float.isNaN(m6068distanceInMinimumTouchTargettz77jQw) || !hitTestResult.isHitInMinimumTouchTargetBetter(m6068distanceInMinimumTouchTargettz77jQw, false)) {
                    return;
                }
                m6060hitNearJHbHoSQ(m6072headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, false, m6068distanceInMinimumTouchTargettz77jQw);
                return;
            }
            return;
        }
        if (m6072headH91voCI == null) {
            mo5995hitTestChildYqVAtuI(hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        if (m6074isPointerInBoundsk4lQ0M(pointerPosition)) {
            m6059hit1hIXUjU(m6072headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        float m6068distanceInMinimumTouchTargettz77jQw2 = !isTouchEvent ? Float.POSITIVE_INFINITY : m6068distanceInMinimumTouchTargettz77jQw(pointerPosition, m6071getMinimumTouchTargetSizeNHjbRc());
        if (!Float.isInfinite(m6068distanceInMinimumTouchTargettz77jQw2) && !Float.isNaN(m6068distanceInMinimumTouchTargettz77jQw2)) {
            if (hitTestResult.isHitInMinimumTouchTargetBetter(m6068distanceInMinimumTouchTargettz77jQw2, isInLayer)) {
                m6060hitNearJHbHoSQ(m6072headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m6068distanceInMinimumTouchTargettz77jQw2);
                return;
            }
        }
        m6063speculativeHitJHbHoSQ(m6072headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m6068distanceInMinimumTouchTargettz77jQw2);
    }

    /* renamed from: hit-1hIXUjU */
    public final void m6059hit1hIXUjU(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2) {
        if (node == null) {
            mo5995hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hit(node, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m6084nextUntilhw7D004;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m6084nextUntilhw7D004 = NodeCoordinatorKt.m6084nextUntilhw7D004(node, hitTestSource.mo6082entityTypeOLwlOKw(), NodeKind.m6086constructorimpl(2));
                    nodeCoordinator.m6059hit1hIXUjU(m6084nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2);
                }
            });
        }
    }
    ...
    public final void m6060hitNearJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        if (node == null) {
            mo5995hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hitInMinimumTouchTarget(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hitNear$1
                ...
                public final void invoke2() {
                    ...
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m6084nextUntilhw7D004 = NodeCoordinatorKt.m6084nextUntilhw7D004(node, hitTestSource.mo6082entityTypeOLwlOKw(), NodeKind.m6086constructorimpl(2));
                    nodeCoordinator.m6060hitNearJHbHoSQ(m6084nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public final void m6063speculativeHitJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        Modifier.Node m6084nextUntilhw7D004;
        if (node == null) {
            mo5995hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else if (!hitTestSource.interceptOutOfBoundsChildEvents(node)) {
            m6084nextUntilhw7D004 = NodeCoordinatorKt.m6084nextUntilhw7D004(node, hitTestSource.mo6082entityTypeOLwlOKw(), NodeKind.m6086constructorimpl(2));
            m6063speculativeHitJHbHoSQ(m6084nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
        } else {
            hitTestResult.speculativeHit(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$speculativeHit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m6084nextUntilhw7D0042;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m6084nextUntilhw7D0042 = NodeCoordinatorKt.m6084nextUntilhw7D004(node, hitTestSource.mo6082entityTypeOLwlOKw(), NodeKind.m6086constructorimpl(2));
                    nodeCoordinator.m6063speculativeHitJHbHoSQ(m6084nextUntilhw7D0042, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public void mo5995hitTestChildYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        NodeCoordinator nodeCoordinator = this.wrapped;
        if (nodeCoordinator != null) {
            nodeCoordinator.m6073hitTestYqVAtuI(hitTestSource, m6057fromParentPosition8S9VItk$default(nodeCoordinator, pointerPosition, false, 2, null), hitTestResult, isTouchEvent, isInLayer);
        }
    }
    ...
    public final Rect touchBoundsInRoot() {
        ...
        long m6067calculateMinimumTouchTargetPaddingE7KxVPU = m6067calculateMinimumTouchTargetPaddingE7KxVPU(m6071getMinimumTouchTargetSizeNHjbRc());
        rectCache.setLeft(-Size.m4343getWidthimpl(m6067calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setTop(-Size.m4340getHeightimpl(m6067calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setRight(getMeasuredWidth() + Size.m4343getWidthimpl(m6067calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setBottom(getMeasuredHeight() + Size.m4340getHeightimpl(m6067calculateMinimumTouchTargetPaddingE7KxVPU));
    ...
    public long mo5865screenToLocalMKHz9U(long relativeToScreen) {
        ...
        return mo5860localPositionOfR5De75A(LayoutCoordinatesKt.findRootCoordinates(this), LayoutNodeKt.requireOwner(getLayoutNode()).mo5805screenToLocalMKHz9U(relativeToScreen));
    }
    ...
    public long mo5863localToScreenMKHz9U(long relativeToLocal) {
        ...
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo5804localToScreenMKHz9U(mo5862localToRootMKHz9U(relativeToLocal));
    }
    ...
    public long mo5868windowToLocalMKHz9U(long relativeToWindow) {
        ...
        LayoutCoordinates findRootCoordinates = LayoutCoordinatesKt.findRootCoordinates(this);
        return mo5860localPositionOfR5De75A(findRootCoordinates, Offset.m4278minusMKHz9U(LayoutNodeKt.requireOwner(getLayoutNode()).mo6139calculateLocalPositionMKHz9U(relativeToWindow), LayoutCoordinatesKt.positionInRoot(findRootCoordinates)));
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: localToWindow-MK-Hz9U */
    public long mo5864localToWindowMKHz9U(long relativeToLocal) {
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo6140calculatePositionInWindowMKHz9U(mo5862localToRootMKHz9U(relativeToLocal));
    }
    ...
    public long mo5861localPositionOfS_NoaFU(LayoutCoordinates sourceCoordinates, long relativeToSource, boolean includeMotionFrameOfReference) {
        if (sourceCoordinates instanceof LookaheadLayoutCoordinates) {
            ...
            return Offset.m4283unaryMinusF1C5BW0(sourceCoordinates.mo5861localPositionOfS_NoaFU(this, Offset.m4283unaryMinusF1C5BW0(relativeToSource), includeMotionFrameOfReference));
        }
        ...
        while (coordinator != findCommonAncestor$ui_release) {
            relativeToSource = coordinator.m6078toParentPosition8S9VItk(relativeToSource, includeMotionFrameOfReference);
            coordinator = coordinator.wrappedBy;
            Intrinsics.checkNotNull(coordinator);
        }
        return m6056ancestorToLocalS_NoaFU(findCommonAncestor$ui_release, relativeToSource, includeMotionFrameOfReference);
    }
    ...
    public void mo5866transformFromEL8BTi8(LayoutCoordinates sourceCoordinates, float[] matrix) {
        ...
        coordinator.onCoordinatesUsed$ui_release();
        NodeCoordinator findCommonAncestor$ui_release = findCommonAncestor$ui_release(coordinator);
        Matrix.m4755resetimpl(matrix);
        coordinator.m6066transformToAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
        m6065transformFromAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
    }
    ...
    public void mo5867transformToScreen58bKbWc(float[] matrix) {
        ...
        m6066transformToAncestorEL8BTi8(toCoordinator(LayoutCoordinatesKt.findRootCoordinates(this)), matrix);
        requireOwner.mo5803localToScreen58bKbWc(matrix);
    }

    /* renamed from: transformToAncestor-EL8BTi8 */
    private final void m6066transformToAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        NodeCoordinator nodeCoordinator = this;
        while (!Intrinsics.areEqual(nodeCoordinator, ancestor)) {
            OwnedLayer ownedLayer = nodeCoordinator.layer;
            if (ownedLayer != null) {
                ownedLayer.mo6138transform58bKbWc(matrix);
            }
            if (!IntOffset.m7124equalsimpl0(nodeCoordinator.getPosition(), IntOffset.INSTANCE.m7135getZeronOccac())) {
                float[] fArr = tmpMatrix;
                Matrix.m4755resetimpl(fArr);
                Matrix.m4766translateimpl$default(fArr, IntOffset.m7125getXimpl(r1), IntOffset.m7126getYimpl(r1), 0.0f, 4, null);
                Matrix.m4763timesAssign58bKbWc(matrix, fArr);
            }
            nodeCoordinator = nodeCoordinator.wrappedBy;
            Intrinsics.checkNotNull(nodeCoordinator);
        }
    ...
    private final void m6065transformFromAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        if (Intrinsics.areEqual(ancestor, this)) {
            return;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        Intrinsics.checkNotNull(nodeCoordinator);
        nodeCoordinator.m6065transformFromAncestorEL8BTi8(ancestor, matrix);
        if (!IntOffset.m7124equalsimpl0(getPosition(), IntOffset.INSTANCE.m7135getZeronOccac())) {
            float[] fArr = tmpMatrix;
            Matrix.m4755resetimpl(fArr);
            Matrix.m4766translateimpl$default(fArr, -IntOffset.m7125getXimpl(getPosition()), -IntOffset.m7126getYimpl(getPosition()), 0.0f, 4, null);
            Matrix.m4763timesAssign58bKbWc(matrix, fArr);
        }
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            ownedLayer.mo6133inverseTransform58bKbWc(matrix);
        }
    }
    ...
    public Rect localBoundingBoxOf(LayoutCoordinates sourceCoordinates, boolean clipBounds) {
        ...
        rectCache.setRight(IntSize.m7167getWidthimpl(sourceCoordinates.mo5859getSizeYbymL2g()));
        rectCache.setBottom(IntSize.m7166getHeightimpl(sourceCoordinates.mo5859getSizeYbymL2g()));
        while (coordinator != findCommonAncestor$ui_release) {
    ...
    private final long m6056ancestorToLocalS_NoaFU(NodeCoordinator ancestor, long r4, boolean includeMotionFrameOfReference) {
        if (ancestor == this) {
            return r4;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        if (nodeCoordinator == null || Intrinsics.areEqual(ancestor, nodeCoordinator)) {
            return m6069fromParentPosition8S9VItk(r4, includeMotionFrameOfReference);
        }
        return m6069fromParentPosition8S9VItk(nodeCoordinator.m6056ancestorToLocalS_NoaFU(ancestor, r4, includeMotionFrameOfReference), includeMotionFrameOfReference);
    }
    ...
    public long mo5862localToRootMKHz9U(long relativeToLocal) {
        ...
        for (NodeCoordinator nodeCoordinator = this; nodeCoordinator != null; nodeCoordinator = nodeCoordinator.wrappedBy) {
            j = m6064toParentPosition8S9VItk$default(nodeCoordinator, j, false, 2, null);
        }
        return j;
    }
    ...
    public static /* synthetic */ long m6064toParentPosition8S9VItk$default(NodeCoordinator nodeCoordinator, long j, boolean z, int i, Object obj) {
        if (obj != null) {
        ...
        return nodeCoordinator.m6078toParentPosition8S9VItk(j, z);
    }

    /* renamed from: toParentPosition-8S9VItk */
    public long m6078toParentPosition8S9VItk(long position, boolean includeMotionFrameOfReference) {
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            position = ownedLayer.mo6135mapOffset8S9VItk(position, false);
        }
        return (includeMotionFrameOfReference || !getIsPlacedUnderMotionFrameOfReference()) ? IntOffsetKt.m7139plusNvtHpc(position, getPosition()) : position;
    }

    /* renamed from: fromParentPosition-8S9VItk$default */
    public static /* synthetic */ long m6057fromParentPosition8S9VItk$default(NodeCoordinator nodeCoordinator, long j, boolean z, int i, Object obj) {
        if (obj != null) {
        ...
        return nodeCoordinator.m6069fromParentPosition8S9VItk(j, z);
    }

    /* renamed from: fromParentPosition-8S9VItk */
    public long m6069fromParentPosition8S9VItk(long position, boolean includeMotionFrameOfReference) {
        if (includeMotionFrameOfReference || !getIsPlacedUnderMotionFrameOfReference()) {
            position = IntOffsetKt.m7137minusNvtHpc(position, getPosition());
        }
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer != null ? ownedLayer.mo6135mapOffset8S9VItk(position, true) : position;
    }

    public final void drawBorder(Canvas canvas, Paint paint) {
        canvas.drawRect(new Rect(0.5f, 0.5f, IntSize.m7167getWidthimpl(getMeasuredSize()) - 0.5f, IntSize.m7166getHeightimpl(getMeasuredSize()) - 0.5f), paint);
    }
    ...
    public final void rectInParent$ui_release(MutableRect bounds, boolean clipBounds, boolean clipToMinimumTouchTargetSize) {
        ...
        if (ownedLayer != null) {
            if (this.isClipping) {
                if (clipToMinimumTouchTargetSize) {
                    long m6071getMinimumTouchTargetSizeNHjbRc = m6071getMinimumTouchTargetSizeNHjbRc();
                    float m4343getWidthimpl = Size.m4343getWidthimpl(m6071getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    float m4340getHeightimpl = Size.m4340getHeightimpl(m6071getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    bounds.intersect(-m4343getWidthimpl, -m4340getHeightimpl, IntSize.m7167getWidthimpl(mo5859getSizeYbymL2g()) + m4343getWidthimpl, IntSize.m7166getHeightimpl(mo5859getSizeYbymL2g()) + m4340getHeightimpl);
                } else if (clipBounds) {
                    bounds.intersect(0.0f, 0.0f, IntSize.m7167getWidthimpl(mo5859getSizeYbymL2g()), IntSize.m7166getHeightimpl(mo5859getSizeYbymL2g()));
                }
                if (bounds.isEmpty()) {
                    return;
                }
        ...
        float m7125getXimpl = IntOffset.m7125getXimpl(getPosition());
        bounds.setLeft(bounds.getLeft() + m7125getXimpl);
        bounds.setRight(bounds.getRight() + m7125getXimpl);
        float m7126getYimpl = IntOffset.m7126getYimpl(getPosition());
        bounds.setTop(bounds.getTop() + m7126getYimpl);
        bounds.setBottom(bounds.getBottom() + m7126getYimpl);
    }
    ...
    public final boolean m6080withinLayerBoundsk4lQ0M(long pointerPosition) {
        if (!OffsetKt.m4291isFinitek4lQ0M(pointerPosition)) {
            return false;
        }
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer == null || !this.isClipping || ownedLayer.mo6134isInLayerk4lQ0M(pointerPosition);
    }

    /* renamed from: isPointerInBounds-k-4lQ0M */
    protected final boolean m6074isPointerInBoundsk4lQ0M(long pointerPosition) {
        float m4274getXimpl = Offset.m4274getXimpl(pointerPosition);
        float m4275getYimpl = Offset.m4275getYimpl(pointerPosition);
        return m4274getXimpl >= 0.0f && m4275getYimpl >= 0.0f && m4274getXimpl < ((float) getMeasuredWidth()) && m4275getYimpl < ((float) getMeasuredHeight());
    }
    ...
    public final NodeCoordinator findCommonAncestor$ui_release(NodeCoordinator other) {
        ...
        if (layoutNode == layoutNode2) {
            ...
            int m6086constructorimpl = NodeKind.m6086constructorimpl(2);
            if (!tail2.getNode().getIsAttached()) {
                InlineClassHelperKt.throwIllegalStateException("visitLocalAncestors called on an unattached node");
            }
            for (Modifier.Node parent = tail2.getNode().getParent(); parent != null; parent = parent.getParent()) {
                if ((parent.getKindSet() & m6086constructorimpl) != 0 && parent == tail) {
                    return other;
                }
            }
            return this;
    ...
    private final long m6061offsetFromEdgeMKHz9U(long pointerPosition) {
        float m4274getXimpl = Offset.m4274getXimpl(pointerPosition);
        float max = Math.max(0.0f, m4274getXimpl < 0.0f ? -m4274getXimpl : m4274getXimpl - getMeasuredWidth());
        float m4275getYimpl = Offset.m4275getYimpl(pointerPosition);
        return OffsetKt.Offset(max, Math.max(0.0f, m4275getYimpl < 0.0f ? -m4275getYimpl : m4275getYimpl - getMeasuredHeight()));
    }

    /* renamed from: calculateMinimumTouchTargetPadding-E7KxVPU */
    protected final long m6067calculateMinimumTouchTargetPaddingE7KxVPU(long minimumTouchTargetSize) {
        return SizeKt.Size(Math.max(0.0f, (Size.m4343getWidthimpl(minimumTouchTargetSize) - getMeasuredWidth()) / 2.0f), Math.max(0.0f, (Size.m4340getHeightimpl(minimumTouchTargetSize) - getMeasuredHeight()) / 2.0f));
    }

    /* renamed from: distanceInMinimumTouchTarget-tz77jQw */
    public final float m6068distanceInMinimumTouchTargettz77jQw(long pointerPosition, long minimumTouchTargetSize) {
        if (getMeasuredWidth() >= Size.m4343getWidthimpl(minimumTouchTargetSize) && getMeasuredHeight() >= Size.m4340getHeightimpl(minimumTouchTargetSize)) {
            return Float.POSITIVE_INFINITY;
        }
        long m6067calculateMinimumTouchTargetPaddingE7KxVPU = m6067calculateMinimumTouchTargetPaddingE7KxVPU(minimumTouchTargetSize);
        float m4343getWidthimpl = Size.m4343getWidthimpl(m6067calculateMinimumTouchTargetPaddingE7KxVPU);
        float m4340getHeightimpl = Size.m4340getHeightimpl(m6067calculateMinimumTouchTargetPaddingE7KxVPU);
        long m6061offsetFromEdgeMKHz9U = m6061offsetFromEdgeMKHz9U(pointerPosition);
        if ((m4343getWidthimpl > 0.0f || m4340getHeightimpl > 0.0f) && Offset.m4274getXimpl(m6061offsetFromEdgeMKHz9U) <= m4343getWidthimpl && Offset.m4275getYimpl(m6061offsetFromEdgeMKHz9U) <= m4340getHeightimpl) {
            return Offset.m4273getDistanceSquaredimpl(m6061offsetFromEdgeMKHz9U);
        }
        return Float.POSITIVE_INFINITY;
    }
    ...
    public final void onMeasured() {
        ...
        if (m6058hasNodeH91voCI(NodeKind.m6086constructorimpl(128))) {
            Snapshot.Companion companion = Snapshot.INSTANCE;
            Snapshot currentThreadSnapshot = companion.getCurrentThreadSnapshot();
            Function1<Object, Unit> readObserver = currentThreadSnapshot != null ? currentThreadSnapshot.getReadObserver() : null;
            Snapshot makeCurrentNonObservable = companion.makeCurrentNonObservable(currentThreadSnapshot);
            try {
                int m6086constructorimpl = NodeKind.m6086constructorimpl(128);
                boolean m6095getIncludeSelfInTraversalH91voCI = NodeKindKt.m6095getIncludeSelfInTraversalH91voCI(m6086constructorimpl);
                if (m6095getIncludeSelfInTraversalH91voCI) {
                    parent = getTail();
                } else {
                    ...
                    if ((headNode.getKindSet() & m6086constructorimpl) != 0) {
                        ...
                        while (delegatingNode != 0) {
                            if (delegatingNode instanceof LayoutAwareModifierNode) {
                                ((LayoutAwareModifierNode) delegatingNode).mo667onRemeasuredozmzZPI(getMeasuredSize());
                            } else if ((delegatingNode.getKindSet() & m6086constructorimpl) != 0 && (delegatingNode instanceof DelegatingNode)) {
                                Modifier.Node delegate = delegatingNode.getDelegate();
                                int i = 0;
                                delegatingNode = delegatingNode;
    ...
    public final void drawContainedDrawModifiers(Canvas canvas, GraphicsLayer graphicsLayer) {
        Modifier.Node m6072headH91voCI = m6072headH91voCI(NodeKind.m6086constructorimpl(4));
        if (m6072headH91voCI == null) {
            performDraw(canvas, graphicsLayer);
        } else {
            getLayoutNode().getMDrawScope$ui_release().m6013draweZhPAX0$ui_release(canvas, IntSizeKt.m7179toSizeozmzZPI(mo5859getSizeYbymL2g()), this, m6072headH91voCI, graphicsLayer);
        }
    }
    ...
    public final boolean shouldSharePointerInputWithSiblings() {
        Modifier.Node headNode = headNode(NodeKindKt.m6095getIncludeSelfInTraversalH91voCI(NodeKind.m6086constructorimpl(16)));
