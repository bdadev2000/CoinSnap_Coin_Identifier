...
public final class PathHitTester {
    ...
    public final boolean m4799containsk4lQ0M(long position) {
        int i;
        int lineWinding;
        if (!this.path.isEmpty() && this.bounds.m4300containsk4lQ0M(position)) {
            ...
            if (((IntervalTree) intervalTree).root != ((IntervalTree) intervalTree).terminator) {
                ...
                while (arrayList.size() > 0) {
                    ...
                    if (node.overlaps(m4275getYimpl, m4275getYimpl)) {
                        T data = node.getData();
                        ...
                        if (i2 == 1) {
                            lineWinding = BezierKt.lineWinding(points, m4274getXimpl, m4275getYimpl);
                        } else if (i2 == 2) {
                            lineWinding = BezierKt.quadraticWinding(points, m4274getXimpl, m4275getYimpl, fArr, fArr2);
                        } else if (i2 == 3) {
                            lineWinding = BezierKt.cubicWinding(points, m4274getXimpl, m4275getYimpl, fArr, fArr2);
                        }
                        i += lineWinding;
                    }
                    if (node.getLeft() != ((IntervalTree) intervalTree).terminator && node.getLeft().getMax() >= m4275getYimpl) {
                        arrayList.add(node.getLeft());
                    }
                    if (node.getRight() != ((IntervalTree) intervalTree).terminator && node.getRight().getMin() <= m4275getYimpl) {
            ...
            if (PathFillType.m4793equalsimpl0(this.path.mo4406getFillTypeRgk1Os(), PathFillType.INSTANCE.m4797getEvenOddRgk1Os())) {
                i &= 1;
            }
            if (i != 0) {
                return true;
