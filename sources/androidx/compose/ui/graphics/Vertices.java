...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m4569toArgb8_81llA(colors.get(i).m4525unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                m4275getYimpl = Offset.m4274getXimpl(packedValue);
            } else {
                m4275getYimpl = Offset.m4275getYimpl(packedValue);
            }
            fArr[i] = m4275getYimpl;
        }
        return fArr;
    }
}
