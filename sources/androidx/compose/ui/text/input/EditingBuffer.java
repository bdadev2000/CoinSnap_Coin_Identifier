...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6458getMinimpl(j);
        this.selectionEnd = TextRange.m6457getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m6458getMinimpl = TextRange.m6458getMinimpl(j);
        int m6457getMaximpl = TextRange.m6457getMaximpl(j);
        if (m6458getMinimpl < 0 || m6458getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + m6458getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m6457getMaximpl < 0 || m6457getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + m6457getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m6458getMinimpl > m6457getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + m6458getMinimpl + " > " + m6457getMaximpl);
        }
    }
    ...
    public final TextRange m6619getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m6448boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m6620getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long m6621updateRangeAfterDeletepWDy79M = EditingBufferKt.m6621updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m6458getMinimpl(m6621updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m6457getMaximpl(m6621updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m6621updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m6621updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m6454getCollapsedimpl(m6621updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m6458getMinimpl(m6621updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m6457getMaximpl(m6621updateRangeAfterDeletepWDy79M2);
            }
        }
    }
