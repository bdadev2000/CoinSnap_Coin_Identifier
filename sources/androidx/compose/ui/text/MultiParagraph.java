...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m6952getMinWidthimpl(j) != 0 || Constraints.m6951getMinHeightimpl(j) != 0) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m6945getHasBoundedHeightimpl(j)) {
                m6949getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m6949getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                m6949getMaxHeightimpl = Constraints.m6949getMaxHeightimpl(j);
            }
            Paragraph m6344Paragraph_EkL_Y = ParagraphKt.m6344Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, m6950getMaxWidthimpl, 0, m6949getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + m6344Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + m6344Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(m6344Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (m6344Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m6950getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m6323paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m4551getUnspecified0d7_KjU();
        }
        multiParagraph.m6330paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m6330paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo6297paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m6329paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo6296paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m6331painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        AndroidMultiParagraphDraw_androidKt.m6714drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(final int start, final int end) {
        ...
        MultiParagraphKt.m6332findParagraphsByRangeSbBc2M(this.paragraphInfoList, TextRangeKt.TextRange(start, end), new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$getPathForRange$2
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                Path.m4785addPathUv8p0NA$default(Path.this, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        });
        return Path;
    }
    ...
    public final int m6326getOffsetForPositionk4lQ0M(long position) {
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m4275getYimpl(position)));
        if (paragraphInfo.getLength() == 0) {
            return paragraphInfo.getStartIndex();
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo6293getOffsetForPositionk4lQ0M(paragraphInfo.m6341toLocalMKHz9U(position)));
    }

    /* renamed from: getRangeForRect-8-6BmAI, reason: not valid java name */
    public final long m6327getRangeForRect86BmAI(Rect rect, int granularity, TextInclusionStrategy inclusionStrategy) {
        ...
        if (this.paragraphInfoList.get(findParagraphByY).getBottom() >= rect.getBottom() || findParagraphByY == CollectionsKt.getLastIndex(this.paragraphInfoList)) {
            ...
            return ParagraphInfo.m6339toGlobalxdX6G0$default(paragraphInfo, paragraphInfo.getParagraph().mo6294getRangeForRect86BmAI(paragraphInfo.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
        }
        int findParagraphByY2 = MultiParagraphKt.findParagraphByY(this.paragraphInfoList, rect.getBottom());
        long m6465getZerod9O1mEE = TextRange.INSTANCE.m6465getZerod9O1mEE();
        while (TextRange.m6453equalsimpl0(m6465getZerod9O1mEE, TextRange.INSTANCE.m6465getZerod9O1mEE()) && findParagraphByY <= findParagraphByY2) {
            ParagraphInfo paragraphInfo2 = this.paragraphInfoList.get(findParagraphByY);
            m6465getZerod9O1mEE = ParagraphInfo.m6339toGlobalxdX6G0$default(paragraphInfo2, paragraphInfo2.getParagraph().mo6294getRangeForRect86BmAI(paragraphInfo2.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
            findParagraphByY++;
        }
        if (TextRange.m6453equalsimpl0(m6465getZerod9O1mEE, TextRange.INSTANCE.m6465getZerod9O1mEE())) {
            return TextRange.INSTANCE.m6465getZerod9O1mEE();
        }
        long m6465getZerod9O1mEE2 = TextRange.INSTANCE.m6465getZerod9O1mEE();
        while (TextRange.m6453equalsimpl0(m6465getZerod9O1mEE2, TextRange.INSTANCE.m6465getZerod9O1mEE()) && findParagraphByY <= findParagraphByY2) {
            ParagraphInfo paragraphInfo3 = this.paragraphInfoList.get(findParagraphByY2);
            m6465getZerod9O1mEE2 = ParagraphInfo.m6339toGlobalxdX6G0$default(paragraphInfo3, paragraphInfo3.getParagraph().mo6294getRangeForRect86BmAI(paragraphInfo3.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
            findParagraphByY2--;
        }
        return TextRange.m6453equalsimpl0(m6465getZerod9O1mEE2, TextRange.INSTANCE.m6465getZerod9O1mEE()) ? m6465getZerod9O1mEE : TextRangeKt.TextRange(TextRange.m6460getStartimpl(m6465getZerod9O1mEE), TextRange.m6455getEndimpl(m6465getZerod9O1mEE2));
    }
    ...
    public final float[] m6325fillBoundingBoxes8ffj60Q(final long range, final float[] array, int arrayStart) {
        requireIndexInRange(TextRange.m6458getMinimpl(range));
        requireIndexInRangeInclusiveEnd(TextRange.m6457getMaximpl(range));
        final Ref.IntRef intRef = new Ref.IntRef();
        intRef.element = arrayStart;
        final Ref.FloatRef floatRef = new Ref.FloatRef();
        MultiParagraphKt.m6332findParagraphsByRangeSbBc2M(this.paragraphInfoList, range, new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$fillBoundingBoxes$1
            /* JADX INFO: Access modifiers changed from: package-private */
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                ...
                long TextRange = TextRangeKt.TextRange(paragraphInfo.toLocalIndex(paragraphInfo.getStartIndex() > TextRange.m6458getMinimpl(j) ? paragraphInfo.getStartIndex() : TextRange.m6458getMinimpl(j)), paragraphInfo.toLocalIndex(paragraphInfo.getEndIndex() < TextRange.m6457getMaximpl(j) ? paragraphInfo.getEndIndex() : TextRange.m6457getMaximpl(j)));
                paragraphInfo.getParagraph().mo6291fillBoundingBoxes8ffj60Q(TextRange, fArr, intRef2.element);
                int m6456getLengthimpl = intRef2.element + (TextRange.m6456getLengthimpl(TextRange) * 4);
                for (int i = intRef2.element; i < m6456getLengthimpl; i += 4) {
                    int i2 = i + 1;
                    fArr[i2] = fArr[i2] + floatRef2.element;
                    int i3 = i + 3;
                    fArr[i3] = fArr[i3] + floatRef2.element;
                }
                intRef2.element = m6456getLengthimpl;
    ...
    public final long m6328getWordBoundaryjx7JFs(int offset) {
        int findParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            findParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(findParagraphByIndex);
        return paragraphInfo.m6340toGlobalxdX6G0(paragraphInfo.getParagraph().mo6295getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)), false);
    }
