package androidx.collection;

import e0.q;

/* loaded from: classes.dex */
public final class LongLongMapKt {
    /* JADX WARN: Type inference failed for: r0v0, types: [java.lang.Object, androidx.collection.LongLongMap] */
    static {
        ?? obj = new Object();
        long[] jArr = ScatterMapKt.f1590a;
        obj.f1455a = jArr;
        long[] jArr2 = LongSetKt.f1465a;
        obj.f1456b = jArr2;
        obj.f1457c = jArr2;
        int g2 = ScatterMapKt.g(0);
        int max = g2 > 0 ? Math.max(7, ScatterMapKt.f(g2)) : 0;
        obj.d = max;
        if (max != 0) {
            jArr = new long[((max + 15) & (-8)) >> 3];
            q.Y(jArr);
        }
        obj.f1455a = jArr;
        int i2 = max >> 3;
        long j2 = 255 << ((max & 7) << 3);
        jArr[i2] = (jArr[i2] & (~j2)) | j2;
        ScatterMapKt.c(obj.d);
        obj.f1456b = new long[max];
        obj.f1457c = new long[max];
    }
}
