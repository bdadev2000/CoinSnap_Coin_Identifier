...
public class RunGroup {
    ...
    public long computeWrapSize(ConstraintWidgetContainer constraintWidgetContainer, int i) {
        ...
        if (contains && contains2) {
            ...
            float f = (float) (this.mFirstRun.mWidget.getBiasPercent(i) > 0.0f ? (((float) j2) / r11) + (((float) j) / (1.0f - r11)) : 0L);
            wrapDimension = this.mFirstRun.start.mMargin + (f * r11) + 0.5f + wrapDimension2 + (f * (1.0f - r11)) + 0.5f;
