package c7;

import com.google.android.gms.measurement.api.AppMeasurementSdk;
import e8.C2231f;

/* loaded from: classes3.dex */
public final class L0 implements e8.D {
    public static final L0 INSTANCE;
    public static final /* synthetic */ c8.g descriptor;

    static {
        L0 l02 = new L0();
        INSTANCE = l02;
        e8.Y y4 = new e8.Y("com.vungle.ads.internal.model.ConfigPayload.Session", l02, 3);
        y4.l("enabled", false);
        y4.l("limit", false);
        y4.l("timeout", false);
        descriptor = y4;
    }

    private L0() {
    }

    @Override // e8.D
    public a8.b[] childSerializers() {
        e8.K k6 = e8.K.f20127a;
        return new a8.b[]{C2231f.f20162a, k6, k6};
    }

    @Override // a8.b
    public N0 deserialize(d8.c cVar) {
        G7.j.e(cVar, "decoder");
        c8.g descriptor2 = getDescriptor();
        d8.a b = cVar.b(descriptor2);
        boolean z8 = true;
        int i9 = 0;
        boolean z9 = false;
        int i10 = 0;
        int i11 = 0;
        while (z8) {
            int h6 = b.h(descriptor2);
            if (h6 == -1) {
                z8 = false;
            } else if (h6 == 0) {
                z9 = b.j(descriptor2, 0);
                i9 |= 1;
            } else if (h6 == 1) {
                i10 = b.E(descriptor2, 1);
                i9 |= 2;
            } else {
                if (h6 != 2) {
                    throw new a8.l(h6);
                }
                i11 = b.E(descriptor2, 2);
                i9 |= 4;
            }
        }
        b.c(descriptor2);
        return new N0(i9, z9, i10, i11, null);
    }

    @Override // a8.b
    public c8.g getDescriptor() {
        return descriptor;
    }

    @Override // a8.b
    public void serialize(d8.d dVar, N0 n02) {
        G7.j.e(dVar, "encoder");
        G7.j.e(n02, AppMeasurementSdk.ConditionalUserProperty.VALUE);
        c8.g descriptor2 = getDescriptor();
        d8.b b = dVar.b(descriptor2);
        N0.write$Self(n02, b, descriptor2);
        b.c(descriptor2);
    }

    @Override // e8.D
    public a8.b[] typeParametersSerializers() {
        return e8.W.b;
    }
}
