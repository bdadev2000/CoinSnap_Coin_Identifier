...
final class ClassValueParametrizedCache<T> implements ParametrizedSerializerCache<T> {
    ...
    public Object mo1727getgIAlus(KClass<Object> key, List<? extends KType> types) {
        Object m145constructorimpl;
        Intrinsics.checkNotNullParameter(key, "key");
        Intrinsics.checkNotNullParameter(types, "types");
        ConcurrentHashMap access$getSerializers$p = ParametrizedCacheEntry.access$getSerializers$p(get(JvmClassMappingKt.getJavaClass((KClass) key)));
        Object obj = access$getSerializers$p.get(types);
        if (obj == null) {
            try {
                Result.Companion companion = Result.INSTANCE;
                m145constructorimpl = Result.m145constructorimpl(this.compute.invoke(key, types));
            } catch (Throwable th2) {
                Result.Companion companion2 = Result.INSTANCE;
                m145constructorimpl = Result.m145constructorimpl(ResultKt.createFailure(th2));
            }
            Result m144boximpl = Result.m144boximpl(m145constructorimpl);
            Object putIfAbsent = access$getSerializers$p.putIfAbsent(types, m144boximpl);
            if (putIfAbsent == null) {
                obj = m144boximpl;
            } else {
                obj = putIfAbsent;
