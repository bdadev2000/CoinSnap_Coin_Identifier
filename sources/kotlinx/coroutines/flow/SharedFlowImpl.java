...
public class SharedFlowImpl<T> extends AbstractSharedFlow<SharedFlowSlot> implements MutableSharedFlow<T>, CancellableFlow<T>, FusibleFlow<T> {
    ...
    public final Object awaitValue(SharedFlowSlot sharedFlowSlot, Continuation<? super Unit> continuation) {
        ...
        synchronized (this) {
            if (tryPeekLocked(sharedFlowSlot) < 0) {
                ...
                cancellableContinuationImpl.resumeWith(Result.m145constructorimpl(Unit.INSTANCE));
            }
    ...
    */
    ...
    public static /* synthetic */ <T> Object emit$suspendImpl(SharedFlowImpl<T> sharedFlowImpl, T t5, Continuation<? super Unit> continuation) {
        if (sharedFlowImpl.tryEmit(t5)) {
            return Unit.INSTANCE;
        }
        Object emitSuspend = sharedFlowImpl.emitSuspend(t5, continuation);
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public final Object emitSuspend(T t5, Continuation<? super Unit> continuation) {
        ...
        for (Continuation<Unit> continuation2 : continuationArr) {
            if (continuation2 != null) {
                ...
                continuation2.resumeWith(Result.m145constructorimpl(Unit.INSTANCE));
            }
        }
        Object result = cancellableContinuationImpl.getResult();
        if (result == IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
    ...
    private final Object tryTakeValue(SharedFlowSlot slot) {
        ...
        for (Continuation<Unit> continuation : continuationArr) {
            if (continuation != null) {
                Result.Companion companion = Result.INSTANCE;
                continuation.resumeWith(Result.m145constructorimpl(Unit.INSTANCE));
            }
        }
        return obj;
    }
    ...
    public final Continuation<Unit>[] updateCollectorIndexLocked$kotlinx_coroutines_core(long oldIndex) {
        ...
        if (getNCollectors() > 0) {
            i10 = Math.min(this.queueSize, this.bufferCapacity - ((int) (bufferEndIndex - j13)));
        } else {
            i10 = this.queueSize;
        }
        Continuation<Unit>[] continuationArr = AbstractSharedFlowKt.EMPTY_RESUMES;
        ...
        updateBufferLocked(max, j11, bufferEndIndex, j10);
        cleanupTailLocked();
