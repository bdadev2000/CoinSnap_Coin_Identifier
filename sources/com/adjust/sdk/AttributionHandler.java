...
public class AttributionHandler implements IAttributionHandler, IActivityPackageSender.ResponseDataCallbackSubscriber {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void getAttributionI(long j3) {
        if (this.timer.getFireIn() > j3) {
            return;
        }
        if (j3 != 0) {
            this.logger.debug("Waiting to query attribution in %s seconds", Util.SecondsDisplayFormat.format(j3 / 1000.0d));
        }
        this.timer.startIn(j3);
    }
