...
public class SharedPreferencesManager {
    ...
    private SharedPreferencesManager(Context context) {
        try {
            ...
            AdjustFactory.getLogger().error("Cannot access to SharedPreferences", e2.getMessage());
    ...
    private synchronized long getLong(String str, long j3) {
        SharedPreferences sharedPreferences2 = sharedPreferences;
        if (sharedPreferences2 == null) {
            return j3;
        }
    ...
    private synchronized int getRawReferrerIndex(String str, long j3) {
        try {
            ...
            for (int i10 = 0; i10 < rawReferrerArray.length(); i10++) {
                ...
                if (optString != null && optString.equals(str) && jSONArray.optLong(1, -1L) == j3) {
                    return i10;
                }
            }
    ...
    private synchronized void saveLong(String str, long j3) {
        SharedPreferences.Editor editor = sharedPreferencesEditor;
        if (editor != null) {
    ...
    public synchronized JSONArray getRawReferrer(String str, long j3) {
        int rawReferrerIndex = getRawReferrerIndex(str, j3);
    ...
    public synchronized void removeRawReferrer(String str, long j3) {
        if (str != null) {
            if (str.length() != 0) {
                int rawReferrerIndex = getRawReferrerIndex(str, j3);
                if (rawReferrerIndex < 0) {
    ...
    public synchronized void saveDeeplink(Uri uri, long j3) {
        if (uri == null) {
            return;
        }
        saveString(PREFS_KEY_DEEPLINK_URL, uri.toString());
        saveLong(PREFS_KEY_DEEPLINK_CLICK_TIME, j3);
    }
    ...
    public synchronized void saveRawReferrer(String str, long j3) {
        if (getRawReferrer(str, j3) != null) {
            return;
        }
        JSONArray rawReferrerArray = getRawReferrerArray();
    ...
    public synchronized void setPreinstallPayloadReadStatus(long j3) {
        saveLong(PREFS_KEY_PREINSTALL_PAYLOAD_READ_STATUS, j3);
    }
