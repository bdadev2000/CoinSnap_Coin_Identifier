...
public final class C1013Ym implements InterfaceC0455Bx {
    ...
    static {
        ...
        A0A = IK.A0i(A03(12, 6, Opcodes.LREM));
        A0B = IK.A0i(A03(18, 9, 56));
        A09 = A0D[8];
    }

    private int A00(int i2) throws A0 {
        if (A07(i2)) {
            return this.A05 ? A0D[i2] : A0C[i2];
        }
        throw new A0(A03(53, 12, 84) + (this.A05 ? A03(105, 2, 104) : A03(103, 2, 3)) + A03(0, 12, 112) + i2);
    }

    private int A01(InterfaceC0456By interfaceC0456By) throws IOException, InterruptedException {
        interfaceC0456By.AFq();
        interfaceC0456By.AEO(this.A06, 0, 1);
        byte b2 = this.A06[0];
        if ((b2 & 131) <= 0) {
            return A00((b2 >> 3) & 15);
        }
        throw new A0(A03(65, 38, 22) + ((int) b2));
    }

    private int A02(InterfaceC0456By interfaceC0456By) throws IOException, InterruptedException {
        if (this.A00 == 0) {
            try {
                this.A01 = A01(interfaceC0456By);
                this.A00 = this.A01;
            } catch (EOFException unused) {
                return -1;
            }
        }
        int AFu = this.A03.AFu(interfaceC0456By, this.A00, true);
        if (AFu == -1) {
            return -1;
        }
    ...
    private void A04() {
        if (!this.A04) {
            this.A04 = true;
            this.A03.A69(Format.A06(null, this.A05 ? A03(Opcodes.LNEG, 12, 72) : A03(107, 10, 2), null, -1, A09, 1, this.A05 ? 16000 : 8000, -1, null, null, 0, null));
        }
    }
    ...
    private boolean A09(InterfaceC0456By interfaceC0456By) throws IOException, InterruptedException {
        if (A0A(interfaceC0456By, A0A)) {
            this.A05 = false;
            interfaceC0456By.AGq(A0A.length);
            return true;
        }
        if (!A0A(interfaceC0456By, A0B)) {
            return false;
        }
        this.A05 = true;
        interfaceC0456By.AGq(A0B.length);
        return true;
    }

    private boolean A0A(InterfaceC0456By interfaceC0456By, byte[] bArr) throws IOException, InterruptedException {
        interfaceC0456By.AFq();
        byte[] bArr2 = new byte[bArr.length];
        interfaceC0456By.AEO(bArr2, 0, bArr.length);
    ...
    public final int AEk(InterfaceC0456By interfaceC0456By, C4 c4) throws IOException, InterruptedException {
        if (interfaceC0456By.A86() != 0 || A09(interfaceC0456By)) {
            A04();
