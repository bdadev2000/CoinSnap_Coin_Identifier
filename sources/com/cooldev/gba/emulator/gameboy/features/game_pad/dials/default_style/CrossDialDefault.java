...
public final class CrossDialDefault implements SimulateMotionDial {
    ...
    public static final class State {
        ...
        static {
            ...
            CROSS_STATE_DOWN_RIGHT = new State("CROSS_STATE_DOWN_RIGHT", 2, companion.fromPolar(0.7853982f, 0.5f, CrossDialDefault.DIAGONAL_STRENGTH, f0.C(1, 0)), new PointF(1.0f, 1.0f));
            CROSS_STATE_DOWN = new State("CROSS_STATE_DOWN", 3, companion.fromPolar(CrossDialDefault.SINGLE_DRAWABLE_ANGLE, 0.5f, 2.0f, f0.B(1)), new PointF(0.0f, 1.0f));
            CROSS_STATE_DOWN_LEFT = new State("CROSS_STATE_DOWN_LEFT", 4, companion.fromPolar(2.3561945f, 0.5f, CrossDialDefault.DIAGONAL_STRENGTH, f0.C(1, 2)), new PointF(-1.0f, 1.0f));
            CROSS_STATE_LEFT = new State("CROSS_STATE_LEFT", 5, companion.fromPolar(3.1415927f, 0.5f, 2.0f, f0.B(2)), new PointF(-1.0f, 0.0f));
            CROSS_STATE_UP_LEFT = new State("CROSS_STATE_UP_LEFT", 6, companion.fromPolar(3.926991f, 0.5f, CrossDialDefault.DIAGONAL_STRENGTH, f0.C(3, 2)), new PointF(-1.0f, -1.0f));
            CROSS_STATE_UP = new State("CROSS_STATE_UP", 7, companion.fromPolar(4.712389f, 0.5f, 2.0f, f0.B(3)), new PointF(0.0f, -1.0f));
            CROSS_STATE_UP_RIGHT = new State("CROSS_STATE_UP_RIGHT", 8, companion.fromPolar(5.4977875f, 0.5f, CrossDialDefault.DIAGONAL_STRENGTH, f0.C(3, 0)), new PointF(1.0f, -1.0f));
            State[] $values = $values();
            $VALUES = $values;
            $ENTRIES = f0.l($values);
        }
    ...
    public CrossDialDefault(@NotNull Context context, @NotNull CrossConfig crossConfig, @NotNull RadialGamePadTheme radialGamePadTheme) {
    ...
    private final State computeStateForPosition(float f2, float f3) {
        Object obj;
        e eVar = new e(m.L(q.K(State.values()), new CrossDialDefault$computeStateForPosition$1(this)));
        if (eVar.hasNext()) {
    ...
    private final void drawBackground(Canvas canvas, float f2) {
        ...
        canvas.drawCircle(this.drawingBox.centerX(), this.drawingBox.centerY(), f2, background.getFillPaint());
    ...
    private final void drawDiagonalDirections(Canvas canvas, float f2) {
        for (State state : getDiagonalStates()) {
            this.compositeButtonPaint.drawCompositeButton(canvas, (state.getAnchor().getNormalizedX() * f2 * 0.75f) + this.drawingBox.centerX(), (state.getAnchor().getNormalizedY() * f2 * 0.75f) + this.drawingBox.centerY(), state == currentState());
        }
    }
    ...
    private final j getDiagonalStates() {
        return m.L(q.K(State.values()), CrossDialDefault$getDiagonalStates$1.INSTANCE);
    }
