...
public class AppLovinStarRatingView extends View {
    ...
    private Path getCombinedPathForAllStarsWithSide(float f9) {
        PointF pointF = new PointF(0.0f, this.paddingDp);
        float f10 = pointF.x + this.paddingDp;
        Path path = new Path();
        for (int i9 = 0; i9 < 5; i9++) {
            float f11 = pointF.y;
            Path starPath = getStarPath(new RectF(f10, f11, f10 + f9, f11 + f9));
            f10 += this.interStarsSpacingDp + f9;
    ...
    private Path getStarPath(RectF rectF) {
        ...
        for (int i9 = 0; i9 < 5; i9++) {
            PointF pointF3 = starPointsOnACircle.get(i9);
            PointF pointF4 = starPointsOnACircle2.get(i9);
    ...
    private List<PointF> getStarPointsOnACircle(PointF pointF, float f9, float f10) {
        float f11 = SUM_OF_CENTRAL_ANGLES_IN_CIRCLE_RADIANS / 5;
        ArrayList arrayList = new ArrayList(5);
        for (int i9 = 0; i9 < 5; i9++) {
            double d2 = f9;
            double d9 = f10;
            arrayList.add(new PointF((float) ((Math.cos(d9) * d2) + pointF.x), (float) ((Math.sin(d9) * d2) + pointF.y)));
            f10 += f11;
        }
        return arrayList;
    }
