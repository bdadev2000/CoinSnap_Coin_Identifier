...
public class ServiceUtils {
    ...
    public static URL convertRequestToUrl(Request<?> request, boolean z) {
        ...
        if (z && urlEncode.startsWith(RemoteSettings.FORWARD_SLASH_STRING)) {
            urlEncode = urlEncode.substring(1);
        }
        String str2 = request.getEndpoint() + (RemoteSettings.FORWARD_SLASH_STRING + urlEncode).replaceAll("(?<=/)/", "%2F");
    ...
    /* JADX WARN: Removed duplicated region for block: B:39:0x00b6 A[ADDED_TO_REGION, ORIG_RETURN, RETURN] */
    /*
