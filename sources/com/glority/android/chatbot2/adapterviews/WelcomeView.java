...
public final class WelcomeView implements IChatbotAdapterView {
    ...
    public int getLayout() {
        return R.layout.chatbot_item_welcome;
    }
    ...
    public void convert(BaseViewHolder helper, Object item, ChatbotUiConfig uiConfig) {
        ...
        int i = R.id.tv;
        if (item == null) {
            throw new NullPointerException("null cannot be cast to non-null type com.glority.android.chatbot2.entity.ChatbotWelcomeEntity");
        }
        ChatbotWelcomeEntity chatbotWelcomeEntity = (ChatbotWelcomeEntity) item;
        helper.setText(i, chatbotWelcomeEntity.getText());
        helper.setImageResource(R.id.iv, chatbotWelcomeEntity.getIcon());
        Integer chatbotTextMsgBackgrouColor = uiConfig.getChatbotTextMsgBackgrouColor();
        if (chatbotTextMsgBackgrouColor != null) {
            helper.getView(R.id.tv).getBackground().setTint(chatbotTextMsgBackgrouColor.intValue());
        }
        Integer chatbotTextColor = uiConfig.getChatbotTextColor();
        if (chatbotTextColor != null) {
            ((TextView) helper.getView(R.id.tv)).setTextColor(chatbotTextColor.intValue());
        }
        Integer textMaxWidth = uiConfig.getTextMaxWidth();
        if (textMaxWidth != null) {
            ((TextView) helper.getView(R.id.tv)).setMaxWidth(textMaxWidth.intValue());
        }
    }

    @Override // com.glority.android.chatbot2.adapterviews.IChatbotAdapterView
    public Class<? extends BaseChatbotEntity> entityClass() {
        return ChatbotWelcomeEntity.class;
    }
}
