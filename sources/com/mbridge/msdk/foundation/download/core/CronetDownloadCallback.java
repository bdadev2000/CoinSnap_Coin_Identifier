...
public class CronetDownloadCallback extends UrlRequest.Callback {
    ...
    private static void cancelRequestSafety(UrlRequest urlRequest) {
        try {
            ...
            if (MBridgeConstans.DEBUG) {
                ad.b(TAG, "cancelRequest error: ", e4);
            }
        }
    }
    ...
    private String getETagFromHeader(List<Map.Entry<String, String>> list) {
        ...
        for (h hVar : responseHeaders) {
            if (hVar != null) {
                String a6 = hVar.a();
                if (!TextUtils.isEmpty(a6) && a6.equalsIgnoreCase(Command.HTTP_HEADER_ETAG)) {
                    return hVar.b();
                }
            }
        }
    ...
    private boolean isSuccessful(int i9) {
        return i9 >= 200 && i9 < 300;
    }

    private boolean isSupportResume(int i9, DownloadModel downloadModel) {
        if (i9 == 206 && !TextUtils.isEmpty(this.etag) && (downloadModel == null || TextUtils.equals(this.etag, downloadModel.getEtag()))) {
            return true;
        }
        return false;
    }
    ...
    public void onFailed(UrlRequest urlRequest, UrlResponseInfo urlResponseInfo, CronetException cronetException) {
        ...
        try {
            ...
            GlobalComponent.getInstance().getLogger().log(TAG, e4.getMessage());
        }
    ...
    public void onReadCompleted(UrlRequest urlRequest, UrlResponseInfo urlResponseInfo, ByteBuffer byteBuffer) throws Exception {
        if (urlRequest != null && urlResponseInfo != null && byteBuffer != null) {
            try {
                ...
                if (MBridgeConstans.DEBUG) {
                    ad.b(TAG, "onReadCompleted error: ", e4);
                }
                cancelRequestSafety(urlRequest);
            }
        }
    ...
    public void onRedirectReceived(UrlRequest urlRequest, UrlResponseInfo urlResponseInfo, String str) throws Exception {
        ...
        try {
            ...
            if (MBridgeConstans.DEBUG) {
                ad.b(TAG, "onRedirectReceived error: ", e4);
            }
        }
    }
    ...
    public void onResponseStarted(UrlRequest urlRequest, UrlResponseInfo urlResponseInfo) throws Exception {
        ...
        if (checkDownloadRequestInfo(urlRequest, urlResponseInfo)) {
            try {
                ...
                if (MBridgeConstans.DEBUG) {
                    ad.b(TAG, "onResponseStarted error: ", e4);
                }
                cancelRequestSafety(urlRequest);
                return;
            }
