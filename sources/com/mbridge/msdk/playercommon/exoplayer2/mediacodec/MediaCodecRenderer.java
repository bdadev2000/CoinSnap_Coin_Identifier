...
public abstract class MediaCodecRenderer extends BaseRenderer {
    ...
    private static boolean codecNeedsEosFlushWorkaround(String str) {
        int i9 = Util.SDK_INT;
        if ((i9 <= 23 && "OMX.google.vorbis.decoder".equals(str)) || (i9 <= 19 && "hb2000".equals(Util.DEVICE) && ("OMX.amlogic.avc.decoder.awesome".equals(str) || "OMX.amlogic.avc.decoder.awesome.secure".equals(str)))) {
            return true;
        }
        return false;
    }
    ...
    private static boolean codecNeedsFlushWorkaround(String str) {
        int i9 = Util.SDK_INT;
        if (i9 >= 18 && ((i9 != 18 || (!"OMX.SEC.avc.dec".equals(str) && !"OMX.SEC.avc.dec.secure".equals(str))) && (i9 != 19 || !Util.MODEL.startsWith("SM-G800") || (!"OMX.Exynos.avc.dec".equals(str) && !"OMX.Exynos.avc.dec.secure".equals(str))))) {
            return false;
        }
        return true;
    }
    ...
    private boolean drainOutputBuffer(long j7, long j9) throws ExoPlaybackException {
        ...
        if (this.codecNeedsEosOutputExceptionWorkaround && this.codecReceivedEos) {
            try {
                ...
                processOutputBuffer = processOutputBuffer(j7, j9, mediaCodec, byteBuffer2, i9, bufferInfo3.flags, bufferInfo3.presentationTimeUs, this.shouldSkipOutputBuffer);
            ...
            processOutputBuffer = processOutputBuffer(j7, j9, mediaCodec2, byteBuffer3, i10, bufferInfo4.flags, bufferInfo4.presentationTimeUs, this.shouldSkipOutputBuffer);
        }
    ...
    private boolean feedInputBuffer() throws ExoPlaybackException {
        ...
        try {
            ...
            if (decoderInputBuffer.isDecodeOnly()) {
                this.decodeOnlyPresentationTimestamps.add(Long.valueOf(j7));
            }
            this.buffer.flip();
            onQueueInputBuffer(this.buffer);
            if (isEncrypted) {
                this.codec.queueSecureInputBuffer(this.inputIndex, 0, getFrameworkCryptoInfo(this.buffer, position), j7, 0);
            } else {
                this.codec.queueInputBuffer(this.inputIndex, 0, this.buffer.data.limit(), j7, 0);
            }
            resetInputBuffer();
            this.codecReceivedBuffers = true;
            this.codecReconfigurationState = 0;
    ...
    private ByteBuffer getInputBuffer(int i9) {
        if (Util.SDK_INT >= 21) {
            return this.codec.getInputBuffer(i9);
        }
        return this.inputBuffers[i9];
    }

    private ByteBuffer getOutputBuffer(int i9) {
        if (Util.SDK_INT >= 21) {
            return this.codec.getOutputBuffer(i9);
        }
        return this.outputBuffers[i9];
    }
    ...
    private boolean shouldSkipOutputBuffer(long j7) {
        int size = this.decodeOnlyPresentationTimestamps.size();
        for (int i9 = 0; i9 < size; i9++) {
            if (this.decodeOnlyPresentationTimestamps.get(i9).longValue() == j7) {
        ...
        if (this.codec == null && (format = this.format) != null) {
            ...
            if (this.codecInfo == null) {
                try {
                    ...
                    throwDecoderInitError(new DecoderInitializationException(this.format, e4, z8, -49998));
                }
                if (this.codecInfo == null) {
            ...
            this.codecHotswapDeadlineMs = j7;
            resetInputBuffer();
            resetOutputBuffer();
            this.waitingForFirstSyncFrame = true;
    ...
    */
    ...
    public abstract boolean processOutputBuffer(long j7, long j9, MediaCodec mediaCodec, ByteBuffer byteBuffer, int i9, int i10, long j10, boolean z8) throws ExoPlaybackException;
    ...
    @Override // com.mbridge.msdk.playercommon.exoplayer2.Renderer
    public void render(long j7, long j9) throws ExoPlaybackException {
        if (this.outputStreamEnded) {
            renderToEndOfStream();
            return;
        }
        ...
        if (this.codec != null) {
            ...
            do {
            } while (drainOutputBuffer(j7, j9));
            do {
            } while (feedInputBuffer());
            TraceUtil.endSection();
        } else {
            DecoderCounters decoderCounters = this.decoderCounters;
            decoderCounters.skippedInputBufferCount = skipSource(j7) + decoderCounters.skippedInputBufferCount;
            this.flagsOnlyBuffer.clear();
    ...
    public static class DecoderInitializationException extends Exception {
        ...
        private static String buildCustomDiagnosticInfo(int i9) {
            ...
            return "com.google.android.exoplayer.MediaCodecTrackRenderer_" + str + Math.abs(i9);
        }
