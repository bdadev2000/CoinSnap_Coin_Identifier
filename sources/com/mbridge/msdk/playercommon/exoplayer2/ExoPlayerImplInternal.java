...
final class ExoPlayerImplInternal implements Handler.Callback, DefaultMediaClock.PlaybackParameterListener, PlayerMessage.Sender, MediaPeriod.Callback, MediaSource.SourceInfoRefreshListener, TrackSelector.InvalidationListener {
    ...
    public static final class PendingMessageInfo implements Comparable<PendingMessageInfo> {
        ...
        public final void setResolvedPosition(int i9, long j7, Object obj) {
    ...
    public static final class SeekPosition {
        ...
        public SeekPosition(Timeline timeline, int i9, long j7) {
            this.timeline = timeline;
            this.windowIndex = i9;
    ...
    private void doSomeWork() throws ExoPlaybackException, IOException {
        ...
        long j7 = playingPeriod.info.durationUs;
        if (z10 && ((j7 == C.TIME_UNSET || j7 <= this.playbackInfo.positionUs) && playingPeriod.info.isFinal)) {
            setState(4);
            stopRenderers();
    ...
    private void enableRenderer(int i9, boolean z8, int i10) throws ExoPlaybackException {
        ...
        if (renderer.getState() == 0) {
            ...
            renderer.enable(rendererConfiguration, formats, playingPeriod.sampleStreams[i9], this.rendererPositionUs, z10, playingPeriod.getRendererOffset());
    ...
    private Pair<Integer, Long> getPeriodPosition(Timeline timeline, int i9, long j7) {
        return timeline.getPeriodPosition(this.window, this.period, i9, j7);
    }
    ...
    private void handleSourceInfoRefreshed(MediaSourceRefreshInfo mediaSourceRefreshInfo) throws ExoPlaybackException {
        ...
        if (i9 > 0) {
            ...
            if (this.playbackInfo.startPositionUs == C.TIME_UNSET) {
                ...
                this.playbackInfo = playbackInfo2.fromNewPosition(resolveMediaPeriodIdForAds2, j7, longValue2);
                return;
            }
            return;
        }
        PlaybackInfo playbackInfo3 = this.playbackInfo;
        int i10 = playbackInfo3.periodId.periodIndex;
        long j11 = playbackInfo3.contentPositionUs;
        if (timeline.isEmpty()) {
            if (!timeline2.isEmpty()) {
                MediaSource.MediaPeriodId resolveMediaPeriodIdForAds3 = this.queue.resolveMediaPeriodIdForAds(i10, j11);
        ...
        if (frontPeriod == null) {
            obj = timeline.getPeriod(i10, this.period, true).uid;
        } else {
            obj = frontPeriod.uid;
        }
        int indexOfPeriod = timeline2.getIndexOfPeriod(obj);
        if (indexOfPeriod == -1) {
            int resolveSubsequentPeriod = resolveSubsequentPeriod(i10, timeline, timeline2);
    ...
    private boolean isTimelineReady() {
        ...
        MediaPeriodHolder playingPeriod = this.queue.getPlayingPeriod();
        long j7 = playingPeriod.info.durationUs;
        if (j7 != C.TIME_UNSET && this.playbackInfo.positionUs >= j7 && ((mediaPeriodHolder = playingPeriod.next) == null || (!mediaPeriodHolder.prepared && !mediaPeriodHolder.info.id.isAd()))) {
            return false;
        }
        return true;
    }
    ...
    private void maybeNotifyPlaybackInfoChanged() {
        ...
        if (this.playbackInfoUpdate.hasPendingUpdate(this.playbackInfo)) {
            ...
            int i10 = this.playbackInfoUpdate.operationAcks;
            if (this.playbackInfoUpdate.positionDiscontinuity) {
                i9 = this.playbackInfoUpdate.discontinuityReason;
            } else {
                i9 = -1;
            }
            handler.obtainMessage(0, i10, i9, this.playbackInfo).sendToTarget();
    ...
    */
    ...
    private void resetInternal(boolean z8, boolean z9, boolean z10) {
        ...
        if (z9) {
            ...
            j7 = this.playbackInfo.positionUs;
        }
        if (!z9) {
        ...
        this.playbackInfo = new PlaybackInfo(timeline2, obj, mediaPeriodId2, j7, j10, i9, false, trackGroupArray2, trackSelectorResult);
    ...
    private void resetRendererPosition(long j7) throws ExoPlaybackException {
        if (this.queue.hasPlayingPeriod()) {
            j7 = this.queue.getPlayingPeriod().toRendererTime(j7);
        }
        this.rendererPositionUs = j7;
        this.mediaClock.resetPosition(j7);
        for (Renderer renderer : this.enabledRenderers) {
            renderer.resetPosition(this.rendererPositionUs);
        }
    }
    ...
    private int resolveSubsequentPeriod(int i9, Timeline timeline, Timeline timeline2) {
        ...
        for (int i12 = 0; i12 < periodCount && i11 == -1; i12++) {
            i10 = timeline.getNextPeriodIndex(i10, this.period, this.window, this.repeatMode, this.shuffleModeEnabled);
            if (i10 == -1) {
                break;
            }
            i11 = timeline2.getIndexOfPeriod(timeline.getPeriod(i10, this.period, true).uid);
        }
        return i11;
    }

    private void scheduleNextWork(long j7, long j9) {
        this.handler.removeMessages(2);
        this.handler.sendEmptyMessageAtTime(2, j7 + j9);
    }
    ...
    */
    ...
    private long seekToPeriodPosition(MediaSource.MediaPeriodId mediaPeriodId, long j7) throws ExoPlaybackException {
        return seekToPeriodPosition(mediaPeriodId, j7, this.queue.getPlayingPeriod() != this.queue.getReadingPeriod());
    }
        ...
        playerMessage.getHandler().post(new Runnable() { // from class: com.mbridge.msdk.playercommon.exoplayer2.ExoPlayerImplInternal.1
            ...
            public void run() {
                try {
                    ...
                    Log.e(ExoPlayerImplInternal.TAG, "Unexpected error delivering message on external thread.", e4);
    ...
    private boolean shouldKeepPeriodHolder(MediaSource.MediaPeriodId mediaPeriodId, long j7, MediaPeriodHolder mediaPeriodHolder) {
    ...
    private void updatePeriods() throws ExoPlaybackException, IOException {
        ...
        while (this.playWhenReady && playingPeriod != readingPeriod && this.rendererPositionUs >= playingPeriod.next.rendererPositionOffsetUs) {
            ...
            this.playbackInfoUpdate.setPositionDiscontinuity(i9);
            updatePlaybackPositions();
                                    ...
                                    if (!z8) {
                                        if (!renderer3.isCurrentStreamFinal()) {
                                            ...
                                            if (isRendererEnabled && rendererConfiguration2.equals(rendererConfiguration) && !z9) {
                                                renderer3.replaceStream(getFormats(trackSelection), advanceReadingPeriod.sampleStreams[i12], advanceReadingPeriod.getRendererOffset());
                                            }
                                        }
                                    }
        ...
        if (playingPeriod != null && mediaPeriodHolder != playingPeriod) {
            ...
            while (true) {
                ...
                if (i9 < rendererArr.length) {
                    ...
                    if (zArr[i9] && (!playingPeriod.trackSelectorResult.isRendererEnabled(i9) || (renderer.isCurrentStreamFinal() && renderer.getStream() == mediaPeriodHolder.sampleStreams[i9]))) {
                        disableRenderer(renderer);
                    }
                    i9++;
                } else {
    ...
    public final boolean handleMessage(Message message) {
        ...
        try {
            ...
            Log.e(TAG, "Playback error.", e4);
            stopInternal(false, false);
            this.eventHandler.obtainMessage(2, e4).sendToTarget();
            maybeNotifyPlaybackInfoChanged();
    ...
    public final synchronized void release() {
        if (!this.released && this.internalPlaybackThread.isAlive()) {
            ...
            long j7 = DEFAULT_RELEASE_TIMEOUT_MS;
            long j9 = elapsedRealtime + DEFAULT_RELEASE_TIMEOUT_MS;
            boolean z8 = false;
            while (!this.released && j7 > 0) {
                try {
                    wait(j7);
                } catch (InterruptedException unused) {
                    z8 = true;
                }
                j7 = j9 - this.clock.elapsedRealtime();
            }
            if (z8) {
    ...
    public final void seekTo(Timeline timeline, int i9, long j7) {
        this.handler.obtainMessage(3, new SeekPosition(timeline, i9, j7)).sendToTarget();
    }
    ...
    private long seekToPeriodPosition(MediaSource.MediaPeriodId mediaPeriodId, long j7, boolean z8) throws ExoPlaybackException {
        stopRenderers();
        this.rebuffering = false;
        setState(2);
        MediaPeriodHolder playingPeriod = this.queue.getPlayingPeriod();
        ...
        while (true) {
            ...
            if (shouldKeepPeriodHolder(mediaPeriodId, j7, mediaPeriodHolder)) {
        ...
        if (mediaPeriodHolder != null) {
            ...
            if (mediaPeriodHolder.hasEnabledTracks) {
                j7 = mediaPeriodHolder.mediaPeriod.seekToUs(j7);
                mediaPeriodHolder.mediaPeriod.discardBuffer(j7 - this.backBufferDurationUs, this.retainBackBufferFromKeyframe);
            }
            resetRendererPosition(j7);
            maybeContinueLoading();
        } else {
