...
class Huffman {
    ...
    private void addCode(int i10, int i11, byte b3) {
        Node node = new Node(i10, b3);
    ...
    public void encode(ByteString byteString, BufferedSink bufferedSink) throws IOException {
        ...
        for (int i11 = 0; i11 < byteString.size(); i11++) {
            int i12 = byteString.getByte(i11) & UByte.MAX_VALUE;
            int i13 = CODES[i12];
            byte b3 = CODE_LENGTHS[i12];
            j3 = (j3 << b3) | i13;
            i10 += b3;
            while (i10 >= 8) {
                i10 -= 8;
                bufferedSink.writeByte((int) (j3 >> i10));
            }
        }
        if (i10 > 0) {
            bufferedSink.writeByte((int) ((j3 << (8 - i10)) | (255 >>> i10)));
        }
    }
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i10 = 0; i10 < byteString.size(); i10++) {
            j3 += CODE_LENGTHS[byteString.getByte(i10) & UByte.MAX_VALUE];
        }
        return (int) ((j3 + 7) >> 3);
    }
