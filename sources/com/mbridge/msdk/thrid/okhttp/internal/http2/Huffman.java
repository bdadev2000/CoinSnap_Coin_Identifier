...
class Huffman {
    ...
    private void addCode(int i9, int i10, byte b) {
        Node node = new Node(i9, b);
        ...
        while (b > 8) {
            ...
            if (nodeArr != null) {
                if (nodeArr[i11] == null) {
                    nodeArr[i11] = new Node();
                }
                node2 = node2.children[i11];
            } else {
    ...
    public byte[] decode(byte[] bArr) {
        ...
        for (byte b : bArr) {
            ...
            i10 += 8;
            while (i10 >= 8) {
                node = node.children[(i9 >>> (i10 - 8)) & 255];
                if (node.children == null) {
                    byteArrayOutputStream.write(node.symbol);
                    i10 -= node.terminalBits;
                    node = this.root;
                } else {
                    i10 -= 8;
                }
        ...
        while (i10 > 0) {
            Node node2 = node.children[(i9 << (8 - i10)) & 255];
            if (node2.children != null || node2.terminalBits > i10) {
                break;
            }
            byteArrayOutputStream.write(node2.symbol);
            i10 -= node2.terminalBits;
            node = this.root;
        }
    ...
    public void encode(ByteString byteString, BufferedSink bufferedSink) throws IOException {
        ...
        for (int i10 = 0; i10 < byteString.size(); i10++) {
            ...
            int i12 = CODES[i11];
            byte b = CODE_LENGTHS[i11];
            j7 = (j7 << b) | i12;
            i9 += b;
            while (i9 >= 8) {
                i9 -= 8;
                bufferedSink.writeByte((int) (j7 >> i9));
            }
        }
        if (i9 > 0) {
            bufferedSink.writeByte((int) ((j7 << (8 - i9)) | (255 >>> i9)));
        }
    }
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i9 = 0; i9 < byteString.size(); i9++) {
            j7 += CODE_LENGTHS[byteString.getByte(i9) & 255];
        }
        return (int) ((j7 + 7) >> 3);
    }
