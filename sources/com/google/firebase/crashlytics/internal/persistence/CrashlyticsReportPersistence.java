...
public class CrashlyticsReportPersistence {
    ...
    private static final Comparator<? super File> LATEST_SESSION_ID_FIRST_COMPARATOR = new androidx.compose.ui.node.a(7);
    private static final FilenameFilter EVENT_FILE_FILTER = new b(5);
    ...
    private static String generateEventFilename(int i2, boolean z2) {
        return androidx.compose.foundation.text.input.a.A("event", String.format(Locale.US, EVENT_COUNTER_FORMAT, Integer.valueOf(i2)), z2 ? PRIORITY_EVENT_SUFFIX : "");
    }
    ...
    private void synthesizeReport(String str, long j2) {
        ...
        if (!arrayList.isEmpty()) {
            synthesizeReportFile(this.fileStore.getSessionFile(str, "report"), arrayList, j2, z2, UserMetadata.readUserId(str, this.fileStore), this.sessionsSubscriber.getAppQualitySessionId(str));
        } else {
            Logger.getLogger().w("Could not parse event files for session " + str);
        }
    }

    private void synthesizeReportFile(@NonNull File file, @NonNull List<CrashlyticsReport.Session.Event> list, long j2, boolean z2, @Nullable String str, @Nullable String str2) {
        try {
            CrashlyticsReportJsonTransform crashlyticsReportJsonTransform = TRANSFORM;
            CrashlyticsReport withEvents = crashlyticsReportJsonTransform.reportFromJson(readTextFile(file)).withSessionEndFields(j2, z2, str).withAppQualitySessionId(str2).withEvents(list);
            CrashlyticsReport.Session session = withEvents.getSession();
            if (session == null) {
                return;
            }
            Logger.getLogger().d("appQualitySessionId: " + str2);
            writeTextFile(z2 ? this.fileStore.getPriorityReport(session.getIdentifier()) : this.fileStore.getReport(session.getIdentifier()), crashlyticsReportJsonTransform.reportToJson(withEvents));
        } catch (IOException e) {
    ...
    private int trimEvents(String str, int i2) {
        List<File> sessionFiles = this.fileStore.getSessionFiles(str, new b(4));
        Collections.sort(sessionFiles, new androidx.compose.ui.node.a(6));
