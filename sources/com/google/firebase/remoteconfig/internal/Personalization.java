        ...
        if (configs.length() >= 1 && (optJSONObject = personalizationMetadata.optJSONObject(str)) != null) {
            ...
            synchronized (this.loggedChoiceIds) {
                try {
                    ...
                    Bundle c2 = androidx.compose.foundation.text.input.a.c(EXTERNAL_RC_PARAMETER_PARAM, str);
                    c2.putString(EXTERNAL_ARM_VALUE_PARAM, configs.optString(str));
                    c2.putString(EXTERNAL_PERSONALIZATION_ID_PARAM, optJSONObject.optString(PERSONALIZATION_ID));
                    c2.putInt(EXTERNAL_ARM_INDEX_PARAM, optJSONObject.optInt(ARM_INDEX, -1));
                    c2.putString("group", optJSONObject.optString("group"));
                    analyticsConnector.logEvent(ANALYTICS_ORIGIN_PERSONALIZATION, EXTERNAL_EVENT, c2);
