...
public final class t {
    ...
    private static long getChannelFieldOffset(Class<?> cls) {
        if (cls != null) {
            try {
                if (d9.hasUnsafeArrayOperations()) {
                    return d9.objectFieldOffset(cls.getDeclaredField("channel"));
                }
                return -1L;
    ...
    private static boolean writeToChannel(ByteBuffer byteBuffer, OutputStream outputStream) throws IOException {
        ...
        if (j3 >= 0 && FILE_OUTPUT_STREAM_CLASS.isInstance(outputStream)) {
            try {
                writableByteChannel = (WritableByteChannel) d9.getObject(outputStream, j3);
