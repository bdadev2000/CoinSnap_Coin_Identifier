...
public class MotionSpec {
    ...
    public static MotionSpec createFromResource(@NonNull Context context, @AnimatorRes int i2) {
        try {
            ...
            Log.w(TAG, "Can't load animation resource ID #0x" + Integer.toHexString(i2), e);
            return null;
        }
    }
    ...
    public long getTotalDuration() {
        ...
        long j2 = 0;
        for (int i2 = 0; i2 < size; i2++) {
            MotionTiming motionTiming = (MotionTiming) this.timings.k(i2);
            j2 = Math.max(j2, motionTiming.getDuration() + motionTiming.getDelay());
        }
        return j2;
    }
