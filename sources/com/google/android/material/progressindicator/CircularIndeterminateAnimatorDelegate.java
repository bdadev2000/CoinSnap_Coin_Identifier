    ...
    private void maybeUpdateSegmentColors(int i2) {
        for (int i3 = 0; i3 < 4; i3++) {
            ...
            if (fractionInRange >= 0.0f && fractionInRange <= 1.0f) {
                ...
                int i5 = iArr[length];
                int i6 = iArr[length2];
                this.activeIndicators.get(0).color = ArgbEvaluatorCompat.getInstance().evaluate(this.interpolator.getInterpolation(fractionInRange), Integer.valueOf(i5), Integer.valueOf(i6)).intValue();
                return;
            }
        }
    }
