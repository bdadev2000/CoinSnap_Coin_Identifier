...
public class HctSolver {
        ...
        for (int i2 = 0; i2 < 3; i2++) {
            ...
            if (d3 != d4) {
                if (d3 < d4) {
                    ...
                    criticalPlaneBelow = criticalPlaneAbove(trueDelinearized(dArr2[i2]));
                } else {
                    criticalPlaneAbove = criticalPlaneAbove(trueDelinearized(d3));
                    criticalPlaneBelow = criticalPlaneBelow(trueDelinearized(dArr2[i2]));
                }
                int i3 = criticalPlaneAbove;
                int i4 = criticalPlaneBelow;
                for (int i5 = 0; i5 < 8 && Math.abs(i4 - i3) > 1; i5++) {
                    int floor = (int) Math.floor((i3 + i4) / 2.0d);
                    double[] coordinate = setCoordinate(dArr, CRITICAL_PLANES[floor], dArr2, i2);
    ...
    public static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i2 = 0; i2 < 12; i2++) {
            double[] nthVertex = nthVertex(d, i2);
            if (nthVertex[0] >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                ...
                if (!z2) {
                    ...
                    d3 = hueOf;
                    d4 = d3;
                } else if (z3 || areInCyclicOrder(d3, hueOf, d4)) {
    ...
    public static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i2 < 5) {
            ...
            double d5 = pow;
            double pow2 = Math.pow(((d2 == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || sqrt == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) ? 0.0d : d2 / Math.sqrt(d4)) * pow, 1.1111111111111112d);
            double d6 = sqrt;
            double pow3 = (Math.pow(d4, (1.0d / viewingConditions.getC()) / viewingConditions.getZ()) * viewingConditions.getAw()) / viewingConditions.getNbb();
            double d7 = (((0.305d + pow3) * 23.0d) * pow2) / (((pow2 * 108.0d) * sin) + (((pow2 * 11.0d) * cos) + (23.0d * ncb)));
            double d8 = d7 * cos;
            double d9 = d7 * sin;
            double d10 = pow3 * 460.0d;
            double[] matrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((288.0d * d9) + ((451.0d * d8) + d10)) / 1403.0d), inverseChromaticAdaptation(((d10 - (891.0d * d8)) - (261.0d * d9)) / 1403.0d), inverseChromaticAdaptation(((d10 - (d8 * 220.0d)) - (d9 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            ...
            double d14 = (dArr[2] * d13) + (dArr[1] * d12) + (dArr[0] * d11);
    ...
    public static double inverseChromaticAdaptation(double d) {
        ...
        return Math.pow(Math.max(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d) * MathUtils.signum(d);
    }
    ...
    public static double[] nthVertex(double d, int i2) {
        double[] dArr = Y_FROM_LINRGB;
        double d2 = dArr[0];
        double d3 = dArr[1];
        double d4 = dArr[2];
        double d5 = i2 % 4 <= 1 ? 0.0d : 100.0d;
        double d6 = i2 % 2 == 0 ? 0.0d : 100.0d;
        if (i2 < 4) {
    ...
    public static double[] setCoordinate(double[] dArr, double d, double[] dArr2, int i2) {
        return lerpPoint(dArr, intercept(dArr[i2], d, dArr2[i2]), dArr2);
    }
