package com.google.android.gms.internal.ads;

import androidx.annotation.Nullable;

/* loaded from: classes2.dex */
public final class zzctd {
    private final zzfgt zza;
    private final zzfgh zzb;
    private final zzcyz zzc;
    private final zzczm zzd;
    private final zzfdv zze;
    private final zzcxt zzf;
    private final zzdck zzg;
    private final zzczq zzh;
    private final zzdfp zzi;

    public zzctd(zzfgt zzfgtVar, zzfgh zzfghVar, zzcyz zzcyzVar, zzczm zzczmVar, @Nullable zzfdv zzfdvVar, zzcxt zzcxtVar, zzdck zzdckVar, zzczq zzczqVar, zzdfp zzdfpVar) {
        this.zza = zzfgtVar;
        this.zzb = zzfghVar;
        this.zzc = zzcyzVar;
        this.zzd = zzczmVar;
        this.zze = zzfdvVar;
        this.zzf = zzcxtVar;
        this.zzg = zzdckVar;
        this.zzh = zzczqVar;
        this.zzi = zzdfpVar;
    }
}
