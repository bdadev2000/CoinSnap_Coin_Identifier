...
public final class zzftr {
    ...
    private static Intent zzc(Intent intent, int i10, int i11) {
        ...
        zzfwq.zzf(z10, "Cannot set any dangerous parts of intent to be mutable.");
        if ((i10 & 1) != 0 && !zzd(0, 3)) {
            z11 = false;
        } else {
            z11 = true;
        }
        zzfwq.zzf(z11, "Cannot use Intent.FILL_IN_ACTION unless the action is marked as mutable.");
        if ((i10 & 2) != 0 && !zzd(0, 5)) {
            z12 = false;
        } else {
            z12 = true;
        }
        zzfwq.zzf(z12, "Cannot use Intent.FILL_IN_DATA unless the data is marked as mutable.");
        if ((i10 & 4) != 0 && !zzd(0, 9)) {
            z13 = false;
        } else {
            z13 = true;
        }
        zzfwq.zzf(z13, "Cannot use Intent.FILL_IN_CATEGORIES unless the category is marked as mutable.");
        if ((i10 & 128) != 0 && !zzd(0, 17)) {
            z14 = false;
        } else {
            z14 = true;
        }
        zzfwq.zzf(z14, "Cannot use Intent.FILL_IN_CLIP_DATA unless the clip data is marked as mutable.");
        if (intent.getComponent() != null) {
            z15 = true;
        } else {
            z15 = false;
        }
        zzfwq.zzf(z15, "Must set component on Intent.");
        if (zzd(0, 1)) {
            zzfwq.zzf(!zzd(i10, 67108864), "Cannot set mutability flags if PendingIntent.FLAG_IMMUTABLE is set.");
        } else {
            zzfwq.zzf(zzd(i10, 67108864), "Must set PendingIntent.FLAG_IMMUTABLE for SDK >= 23 if no parts of intent are mutable.");
        }
