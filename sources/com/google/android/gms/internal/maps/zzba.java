...
public final class zzba {
    public static int zza(int i, int i2, String str) {
        ...
        if (i < 0) {
            zza = zzbb.zza("%s (%s) must not be negative", "index", Integer.valueOf(i));
        } else {
            if (i2 < 0) {
                throw new IllegalArgumentException("negative size: " + i2);
            }
            zza = zzbb.zza("%s (%s) must be less than size (%s)", "index", Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    public static void zzc(int i, int i2, int i3) {
        ...
        if (i < 0 || i2 < i || i2 > i3) {
            if (i < 0 || i > i3) {
                ...
                zzd = zzbb.zza("end index (%s) must not be less than start index (%s)", Integer.valueOf(i2), Integer.valueOf(i));
            }
