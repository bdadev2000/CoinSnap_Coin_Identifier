...
public final class ClassPath {
        ...
        if (value != null) {
            for (String str : CLASS_PATH_ATTRIBUTE_SEPARATOR.split(value)) {
                try {
                    ...
                    if (classPathEntry.getProtocol().equals(TransferTable.COLUMN_FILE)) {
    ...
    static ImmutableMap<File, ClassLoader> getClassPathEntries(ClassLoader classLoader) {
        ...
        while (it.hasNext()) {
            URL next = it.next();
            if (next.getProtocol().equals(TransferTable.COLUMN_FILE)) {
        ...
        for (String str : Splitter.on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
                    ...
                    builder.add((ImmutableList.Builder) new URL(TransferTable.COLUMN_FILE, (String) null, new File(str).getAbsolutePath()));
                }
    ...
    static File toFile(URL url) {
        Preconditions.checkArgument(url.getProtocol().equals(TransferTable.COLUMN_FILE));
        try {
