    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int i2 = 1; i2 < 5; i2++) {
            ...
            for (int i4 = width / 5; i4 < i3; i4++) {
                int i5 = (row[i4] & UnsignedBytes.MAX_VALUE) >> 3;
                iArr[i5] = iArr[i5] + 1;
            }
        }
        ...
        for (int i6 = 0; i6 < height; i6++) {
            ...
            for (int i8 = 0; i8 < width; i8++) {
                if ((matrix[i7 + i8] & UnsignedBytes.MAX_VALUE) < estimateBlackPoint) {
                    bitMatrix.set(i8, i6);
                }
            }
        }
    ...
    public BitArray getBlackRow(int i2, BitArray bitArray) throws NotFoundException {
        ...
        if (width < 3) {
            for (int i5 = 0; i5 < width; i5++) {
                if ((row[i5] & UnsignedBytes.MAX_VALUE) < estimateBlackPoint) {
                    bitArray.set(i5);
                }
            }
        } else {
            int i6 = 1;
            int i7 = row[0] & UnsignedBytes.MAX_VALUE;
            int i8 = row[1] & UnsignedBytes.MAX_VALUE;
            while (i6 < width - 1) {
                int i9 = i6 + 1;
                int i10 = row[i9] & UnsignedBytes.MAX_VALUE;
