...
public final class Detector {
    ...
    private ResultPoint correctTopRightRectangular(ResultPoint resultPoint, ResultPoint resultPoint2, ResultPoint resultPoint3, ResultPoint resultPoint4, int i2, int i3) {
        ...
        if (isValid(resultPoint5)) {
            if (isValid(resultPoint6)) {
                return Math.abs(i3 - transitionsBetween(resultPoint2, resultPoint5).getTransitions()) + Math.abs(i2 - transitionsBetween(resultPoint3, resultPoint5).getTransitions()) <= Math.abs(i3 - transitionsBetween(resultPoint2, resultPoint6).getTransitions()) + Math.abs(i2 - transitionsBetween(resultPoint3, resultPoint6).getTransitions()) ? resultPoint5 : resultPoint6;
            }
            return resultPoint5;
        }
        if (isValid(resultPoint6)) {
    ...
    private ResultPointsAndTransitions transitionsBetween(ResultPoint resultPoint, ResultPoint resultPoint2) {
        ...
        int i2 = 0;
        boolean z2 = Math.abs(y3 - y2) > Math.abs(x2 - x);
        if (z2) {
            y2 = x;
            x = y2;
            y3 = x2;
    ...
    public DetectorResult detect() throws NotFoundException {
        ...
        if (i2 * 4 >= i3 * 7 || i3 * 4 >= i2 * 7) {
            ...
            int max = Math.max(transitionsBetween(r6, resultPoint6).getTransitions(), transitionsBetween(r14, resultPoint6).getTransitions());
            int i5 = max + 1;
            int i6 = (i5 & 1) == 1 ? max + 2 : i5;
            sampleGrid = sampleGrid(this.image, r6, r22, r14, resultPoint6, i6, i6);
            resultPoint = r6;
        }
