...
public final class ViewComponentManager implements GeneratedComponentManager<Object> {
    ...
    private GeneratedComponentManager<?> getParentComponentManager(boolean allowMissing) {
        if (this.hasFragmentBindings) {
            ...
            Preconditions.checkState(!(r4 instanceof GeneratedComponentManager), "%s, @WithFragmentBindings Hilt view must be attached to an @AndroidEntryPoint Fragment. Was attached to context %s", this.view.getClass(), getParentContext(GeneratedComponentManager.class, allowMissing).getClass().getName());
        } else {
            Object parentContext2 = getParentContext(GeneratedComponentManager.class, allowMissing);
