...
public final class InstrSupport {
    ...
    public static int getMajorVersion(byte[] bArr) {
        return (bArr[7] & UnsignedBytes.MAX_VALUE) | ((bArr[6] & UnsignedBytes.MAX_VALUE) << 8);
    }

    public static boolean needsFrames(int i2) {
        return (i2 & Settings.DEFAULT_INITIAL_WINDOW_SIZE) >= 50;
    }

    public static void push(MethodVisitor methodVisitor, int i2) {
        if (i2 >= -1 && i2 <= 5) {
            methodVisitor.visitInsn(i2 + 3);
            return;
        }
        if (i2 >= -128 && i2 <= 127) {
    ...
    public static void setMajorVersion(int i2, byte[] bArr) {
        bArr[6] = (byte) (i2 >>> 8);
        bArr[7] = (byte) i2;
    }
