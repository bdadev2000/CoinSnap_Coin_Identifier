        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(pop)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        SimpleTypeMarker mo9557transformType = none.mo9557transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(mo9557transformType) && !typeSystemContext.isMarkedNullable(mo9557transformType)) || typeSystemContext.isDefinitelyNotNullType(mo9557transformType)) {
                            typeCheckerState.clear();
                            return true;
                        }
                        supertypesDeque.add(mo9557transformType);
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(pop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(pop)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker mo9557transformType = supertypesPolicy.mo9557transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, mo9557transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo9557transformType);
